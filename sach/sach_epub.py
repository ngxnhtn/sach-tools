#!/usr/bin/env python3
"""
Defines the SachEpub class, the master class for representing and operating on Vietnamese epub3 files.
"""
...
import sach.images
from sach.sach_epub_lint import LintMessage
...
class SachEpub:
	...
	def __init__(self, epub_root_directory: str | Path):
		try:
			...
			raise sach.InvalidSachEbookException(f"Not a directory: [path][link=file://{self.path}]{self.path}[/][/].") from ex
		...
		try:
			...
			raise sach.InvalidSachEbookException("Target doesn’t appear to be an epub: no [path]container.xml[/] or no metadata file.") from ex
		...
		try:
			...
			raise sach.InvalidXmlException(f"Couldn’t parse [path][link=file://{self.metadata_file_path}]{self.metadata_file_path}[/][/]: {ex}") from ex
		...
		try:
			...
			raise sach.InvalidSachEbookException("Couldn’t find table of contents.") from ex
	...
	def repo(self) -> Repo:
		...
		if not self._repo:
			try:
				...
				raise sach.InvalidSachEbookException("Couldn’t access this ebook’s Git repository.") from ex
			...
			if self.endnotes_path is not None:
				...
				for node in dom.xpath("/html/body//section[re:test(@epub:type, '\\bendnotes\\b')]/ol/li"):
					...
					try:
						...
						raise sach.InvalidInputException(f"No backlink found in note {note.anchor} in existing endnotes file.") from ex
		...
		if not self._spine_file_paths:
			...
			for idref in self.metadata_dom.xpath("/package/spine/itemref/@idref", str):
				try:
					...
					raise sach.InvalidSachEbookException(f"Couldn’t find spine item: {idref}") from ex
	...
	def generate_identifier(self) -> str:
		...
		for author in self.metadata_dom.xpath("/package/metadata/dc:creator"):
			...
			identifier += sach.formatting.make_url_safe(author.text) + "_"

		identifier = identifier.strip("_") + "/"

		# Add title.
		for title in self.metadata_dom.xpath("/package/metadata/dc:title[@id=\"title\"]"):
			identifier += sach.formatting.make_url_safe(title.text) + "/"
		...
		for translator in translators:
			if (not translators_have_display_seq and translator.include) or translator.display_seq:
				identifier += sach.formatting.make_url_safe(translator.name) + "_"

		if translators:
			identifier = identifier.strip("_") + "/"

		for editor in editors:
			identifier += sach.formatting.make_url_safe(editor.name) + "_"
		...
		for illustrator in illustrators:
			...
			if include_illustrator and (illustrator.include or illustrator.display_seq):
				identifier += sach.formatting.make_url_safe(illustrator.name) + "_"
	...
	def generate_title_string(self) -> str:
		...
		if authors and authors[0] != "Anonymous":
			output += ", by " + sach.formatting.format_list(authors)

		translators = self.get_display_contributors("trl", ignore_list=authors)

		if translators:
			output += ". Translated by " + sach.formatting.format_list(translators)

		editors = self.get_display_contributors("edt", ignore_list=authors)

		if editors:
			output += ". Edited by " + sach.formatting.format_list(editors)

		illustrators = self.get_display_contributors("ill", ignore_list=authors + translators + editors)

		if illustrators:
			output += ". Illustrated by " + sach.formatting.format_list(illustrators)
			...
			if contributor_element:
				...
				if ignore_list:
					for item in ignore_list:
						if item.replace(sach.NO_BREAK_SPACE, " ") == contributor_name.replace(sach.NO_BREAK_SPACE, " "):
				...
				if use_nbsp:
					contributor_name = contributor_name.replace(" ", sach.NO_BREAK_SPACE)
	...
	def get_file(self, file_path: Path) -> str:
		...
		if file_path_str not in self._file_cache:
			try:
				...
				raise sach.InvalidFileException(f"Couldn’t read file: [path]{file_path_str}[/]") from ex
	...
	def get_dom(self, file_path: Path, remove_comments: bool=False) -> EasyXmlTree:
		...
		if file_path_str not in self._dom_cache:
			try:
				...
				raise sach.InvalidXhtmlException(f"Couldn’t parse XML in [path][link=file://{file_path.resolve()}]{file_path}[/][/]: {ex}") from ex
			except FileNotFoundError as ex:
				raise ex
			except sach.InvalidXmlException as ex:
				raise sach.InvalidXhtmlException(f"Couldn’t parse XML in [path][link=file://{file_path.resolve()}]{file_path}[/][/]: {ex.__cause__}") from ex
			except Exception as ex:
				raise sach.InvalidXhtmlException(f"Couldn’t parse XML in [path][link=file://{file_path.resolve()}]{file_path}[/][/].") from ex
	...
	def _recompose_xhtml(self, section: EasyXmlElement, output_dom: EasyXmlTree, use_image_files: bool = False) -> None:
		...
		if not section.get_attr("id") or (section.parent and section.parent.tag.lower() != "body" and not section.parent.get_attr("id")):
			raise sach.InvalidXhtmlException(f"Section without [attr]@id[/] attribute: [xhtml]{section.to_tag_string()}[/]")
		...
		if not use_image_files:
			for img in section.xpath("//img[starts-with(@src, '../images/')]"):
				img.set_attr("src", sach.images.get_data_url(self.content_path / img.get_attr("src").replace("../", "")))
	...
	def _scope_recompose_css_tokens(self, tokens: list[Node], css_class: str) -> str:
		...
		for token in tokens:
			if isinstance(token, ParseError):
				raise sach.InvalidCssException(token.message)
	...
	def recompose(self, output_xhtml5: bool, extra_css_file: Path | None = None, use_image_files: bool = False) -> str:
		...
		try:
			...
			raise sach.InvalidSachEbookException("Couldn’t determine ebook title.") from ex
		...
		for file_path in self.spine_file_paths:
			...
			for node in dom.xpath("/html/head/link[re:test(@rel, '\\bstylesheet\\b') and @href]"):
				...
				css_classes_by_file_path[file_path].append(sach.formatting.make_url_safe(css_filename.name))
		...
		for filepath in css_filenames:
			...
			if not use_image_files:
				for image in regex.finditer(pattern=r"""url\("(.+?\.(?:svg|png|jpg))"\)""", string=file_css):
					...
					url = regex.sub(r"^/", "", url)
					try:
						data_url = sach.images.get_data_url(self.content_path / url)
			...
			if not extra_css_file or filepath != extra_css_file:
				...
				file_css = self._scope_recompose_css_tokens(tokens, sach.formatting.make_url_safe(filepath.name))
		...
		# It occurs at least in `sach.css`.
		...
		output_dom = sach.formatting.EasyXmlTree(output_xhtml)
		...
		for file_path in self.spine_file_paths:
			...
			for node in dom.xpath("/html/body/*[name() = 'section' or name() = 'article']"):
				for css_class in css_classes_by_file_path[file_path]:
					if not sach.formatting.has_css_class(node.get_attr("class"), css_class):
			...
			for node in dom.xpath("/html/body/*"):
				try:
					...
					raise sach.SachException(f"[path][link=file://{file_path}]{file_path}[/][/]: {ex}") from ex
		...
		output_xhtml = sach.formatting.format_xhtml(output_xhtml)
	...
	def generate_titlepage_svg(self) -> None:
		...
		if translators:
			contributors["translated by"] = sach.formatting.format_list(translators)

		editors = self.get_display_contributors("edt", True, authors)

		if editors:
			contributors["edited by"] = sach.formatting.format_list(editors)

		illustrators = self.get_display_contributors("ill", True, authors + translators + editors)

		if illustrators:
			contributors["illustrated by"] = sach.formatting.format_list(illustrators)
		...
		with importlib.resources.files("sach.data.templates").joinpath("titlepage.svg").open("r", encoding="utf-8") as file:
			svg = file.read()
		...
		canvas_width = sach.images.COVER_TITLE_BOX_WIDTH - (sach.images.COVER_TITLE_BOX_HORIZONTAL_PADDING * 2)
		title_lines = sach.images.calculate_image_lines(title.upper(), sach.images.COVER_TITLE_HEIGHT, canvas_width)

		# Now reset the canvas width to the full width of the titlepage canvas for author/contributor lines.
		canvas_width = sach.TITLEPAGE_WIDTH - (sach.images.TITLEPAGE_HORIZONTAL_PADDING * 2)

		# Calculate the author lines.
		authors_lines: list[list[str]] = []
		for author in authors:
			authors_lines.append(sach.images.calculate_image_lines(author.upper(), sach.images.TITLEPAGE_AUTHOR_HEIGHT, canvas_width))
		...
		for descriptor, contributor_name in contributors.items():
			contributor_blocks.append(ContributorsBlock(descriptor, sach.images.calculate_image_lines(contributor_name.upper(), sach.images.TITLEPAGE_CONTRIBUTOR_HEIGHT, canvas_width)))

		# Construct the output.
		text_elements = ""
		element_y = sach.images.TITLEPAGE_VERTICAL_PADDING

		# Add the title.
		i = 0
		for line in title_lines:
			if self._does_line_require_vertical_offset(line, title_lines[i - 1] if i > 0 else None):
				element_y += floor(sach.images.COVER_TITLE_HEIGHT / sach.images.LEAGUE_SPARTAN_DIACRITIC_RATIO)

			element_y += sach.images.TITLEPAGE_TITLE_HEIGHT
			text_elements += f"\t<text class=\"title\" x=\"700\" y=\"{element_y:.0f}\">{escape(line)}</text>\n"
			element_y += sach.images.TITLEPAGE_TITLE_MARGIN
		...
		# Add the author(s).
		if authors:
			element_y += sach.images.TITLEPAGE_AUTHOR_SPACING

		for author_lines in authors_lines:
			for line in author_lines:
				element_y += sach.images.TITLEPAGE_AUTHOR_HEIGHT
				text_elements += f"\t<text class=\"author\" x=\"700\" y=\"{element_y:.0f}\">{escape(line)}</text>\n"
				element_y += sach.images.TITLEPAGE_AUTHOR_MARGIN
		...
		if contributor_blocks:
			element_y += sach.images.TITLEPAGE_CONTRIBUTORS_SPACING
			for contributor_block in contributor_blocks:
				element_y += sach.images.TITLEPAGE_CONTRIBUTOR_DESCRIPTOR_HEIGHT
				text_elements += f"\t<text class=\"contributor-descriptor\" x=\"700\" y=\"{element_y:.0f}\">{escape(contributor_block.descriptor)}</text>\n"
				element_y += sach.images.TITLEPAGE_CONTRIBUTOR_MARGIN

				for person in contributor_block.names:
					element_y += sach.images.TITLEPAGE_CONTRIBUTOR_HEIGHT
					line = person.replace(sach.NO_BREAK_SPACE, " ")
					text_elements += f"\t<text class=\"contributor\" x=\"700\" y=\"{element_y:.0f}\">{escape(line)}</text>\n"
					element_y += sach.images.TITLEPAGE_CONTRIBUTOR_MARGIN

				element_y -= sach.images.TITLEPAGE_CONTRIBUTOR_MARGIN

				element_y += sach.images.TITLEPAGE_CONTRIBUTOR_DESCRIPTOR_MARGIN
		...
		svg = regex.sub(r"viewBox=\".+?\"", f"viewBox=\"0 0 {sach.TITLEPAGE_WIDTH} {element_y:.0f}\"", svg)
	...
	def build_titlepage_svg(self) -> None:
		...
		if source_titlepage_svg_filename.is_file():
			...
			sach.images.svg_text_to_paths(source_titlepage_svg_filename, dest_titlepage_svg_filename)
	...
	def _get_cover_title_box_contents_height(self, title_lines: list[str], title_line_height: int, author_lines: list[list[str]]) -> int:
		...
		spacing = sach.images.COVER_AUTHOR_SPACING
		...
		if title_line_height != sach.images.COVER_TITLE_XSMALL_HEIGHT:
			i = 0
			for line in title_lines:
				if self._does_line_require_vertical_offset(line, title_lines[i - 1] if i > 0 else None):
		...
		return (title_line_count * title_line_height) \
			+ lines_with_diacritic_offset * floor(title_line_height / sach.images.LEAGUE_SPARTAN_DIACRITIC_RATIO) \
 			+ ( (title_line_count - 1) * sach.images.COVER_TITLE_MARGIN) \
 			+ spacing \
 			+ (author_line_count * sach.images.COVER_AUTHOR_HEIGHT) \
 			+ ( additional_author_line_count * sach.images.COVER_AUTHOR_MARGIN)
	...
	def generate_cover_svg(self) -> None:
		...
		# Read our template SVG to get some values before we begin.
		with importlib.resources.files("sach.data.templates").joinpath("cover.svg").open("r", encoding="utf-8") as file:
			svg = file.read()
		...
		authors_lines: list[list[str]] = []
		for author in authors:
			authors_lines.append(sach.images.calculate_image_lines(author.upper(), sach.images.COVER_AUTHOR_HEIGHT, canvas_width))

		# Calculate the title lines.
		title_upper = title.upper()
		title_height = sach.images.COVER_TITLE_HEIGHT
		title_class = "title"
		title_lines = sach.images.calculate_image_lines(title_upper, title_height, canvas_width)

		# Construct the output.
		text_elements = ""

		# Decide if we have to shrink the title text to fit the title box.
		max_cover_title_box_canvas_height = sach.images.COVER_TITLE_BOX_HEIGHT - (sach.images.COVER_TITLE_BOX_VERTICAL_PADDING * 2)
		cover_title_box_contents_height = self._get_cover_title_box_contents_height(title_lines, title_height, authors_lines)
		cover_title_box_contents_width = sach.images.get_image_lines_width(title_lines, title_height)
		...
		while (cover_title_box_contents_height > max_cover_title_box_canvas_height or cover_title_box_contents_width > canvas_width) and title_class != "title-xsmall":
			if title_class == "title-small":
				...
				title_height = sach.images.COVER_TITLE_XSMALL_HEIGHT

			if title_class == "title":
				title_class = "title-small"
				title_height = sach.images.COVER_TITLE_SMALL_HEIGHT

			title_lines = sach.images.calculate_image_lines(title_upper, title_height, canvas_width)
			cover_title_box_contents_height = self._get_cover_title_box_contents_height(title_lines, title_height, authors_lines)
			cover_title_box_contents_width = sach.images.get_image_lines_width(title_lines, title_height)

		element_y = sach.images.COVER_TITLE_BOX_Y + \
			+ ((sach.images.COVER_TITLE_BOX_HEIGHT \
		...
		for line in title_lines:
			# xsmall sizing doesn't require an offset.
			if title_height != sach.images.COVER_TITLE_XSMALL_HEIGHT:
				if self._does_line_require_vertical_offset(line, title_lines[i - 1] if i > 0 else None):
					element_y += floor(title_height / sach.images.LEAGUE_SPARTAN_DIACRITIC_RATIO)

			element_y += title_height
			text_elements += f"\t<text class=\"{title_class}\" x=\"700\" y=\"{element_y:.0f}\">{escape(line)}</text>\n"
			element_y += sach.images.COVER_TITLE_MARGIN
			i = i + 1
		...
		# Add the author(s).
		if authors:
			element_y += sach.images.COVER_AUTHOR_SPACING

			for author_lines in authors_lines:
				for line in author_lines:
					element_y += sach.images.COVER_AUTHOR_HEIGHT
					text_elements += f"\t<text class=\"author\" x=\"700\" y=\"{element_y:.0f}\">{escape(line)}</text>\n"
					element_y += sach.images.COVER_AUTHOR_MARGIN
	...
	def build_cover_svg(self) -> None:
		...
		if source_cover_jpg_filename.is_file() and source_cover_svg_filename.is_file():
			...
			if source_cover_svg_filename.is_file():
				sach.images.svg_text_to_paths(source_cover_svg_filename, dest_cover_svg_filename, remove_style=False)
	...
	def shift_endnotes(self, target_endnote_number: int, step: int = 1) -> None:
		...
		try:
			...
			raise sach.InvalidSachEbookException(f"Couldn’t open endnotes file: [path][link=file://{self.endnotes_path}]{self.endnotes_path}[/][/].") from ex
	...
	def shift_illustrations(self, target_illustration_number: int, step: int = 1) -> None:
		...
		for illustration_number in illustration_numbers:
			...
			for illustration_path in [self.path / "images", self.content_path / "images"]:
				...
				if existing_file:
					raise sach.FileExistsException(f"Couldn’t rename illustration to already existing file: [path][link=file://{existing_file}]{existing_file}[/][/]")
		...
		try:
			...
			raise sach.InvalidSachEbookException(f"Couldn’t open LoI file: [path][link=file://{self.loi_path}]{self.loi_path}[/][/].") from ex
	...
	def generate_loi(self) -> str:
		...
		if not self.loi_path:
			with importlib.resources.files("sach.data.templates").joinpath("loi.xhtml").open("rb") as file:
		...
		if len(ols) != 1:
			raise sach.InvalidSachEbookException(f"LoI contains unexpected number of [xhtml]<ol/>[/]: [path][link=file://{self.loi_path}]{self.loi_path}[/][/].")
			...
			for figure in dom.xpath("/html/body//figure[@id and ./img]"):
				...
				if figcaption:
					...
					if figcaption_text and not regex.search(r"^[Ff]igure\s+\d+$", figcaption_text):
						has_block = False
						for tag in sach.css.CSS_BLOCK_ELEMENTS:
		...
		return sach.formatting.format_xhtml(loi_dom.to_string())
	...
	def set_release_timestamp(self) -> None:
		...
		if self.metadata_dom.xpath("/package/metadata/dc:date[text() = '1900-01-01T00:00:00Z' or text() = '']"):
			...
			now_iso = sach.formatting.generate_iso_timestamp(now)
			now_friendly = sach.formatting.generate_colophon_timestamp(now)
	...
	def update_flesch_reading_ease(self) -> None:
		...
		for filename in sach.get_target_filenames([self.path], ".xhtml"):
			xhtml = self.get_file(filename)

			is_ignored, _ = sach.get_dom_if_not_ignored(xhtml, ["colophon", "titlepage", "imprint", "copyright-page", "halftitlepage", "toc", "loi"])

			if not is_ignored:
				text += xhtml

		for node in self.metadata_dom.xpath("/package/metadata/meta[@property='schema:educationalLevel']"):
			node.set_text(str(sach.formatting.get_flesch_reading_ease(text)))
	...
	def get_word_count(self) -> int:
		...
		for filename in sach.get_target_filenames([self.path], ".xhtml"):
			...
			is_ignored, _ = sach.get_dom_if_not_ignored(xhtml, ["colophon", "titlepage", "imprint", "copyright-page", "halftitlepage", "toc", "loi", "endnotes"])

			if not is_ignored:
				word_count += sach.formatting.get_word_count(xhtml)
	...
	def lint(self, skip_lint_ignore: bool, allowed_messages: list[str] | None = None) -> list[LintMessage]:
		...
		from sach.sach_epub_lint import lint # pylint: disable=import-outside-toplevel,cyclic-import
	...
	def build(self, run_epubcheck: bool, check_only: bool, build_kobo: bool, build_kindle: bool, output_directory: Path, proof: bool, build_cache_directory: Path|None) -> None:
		...
		from sach.sach_epub_build import build # pylint: disable=import-outside-toplevel
	...
	def generate_toc(self) -> str:
		...
		from sach.sach_epub_generate_toc import generate_toc  # pylint: disable=import-outside-toplevel

		toc_xhtml = generate_toc(self)

		# Word joiners and `nbsp` don't go in the ToC.
		toc_xhtml = toc_xhtml.replace(sach.WORD_JOINER, "")
		toc_xhtml = toc_xhtml.replace(sach.NO_BREAK_SPACE, " ")
	...
	def generate_endnotes(self) -> tuple[int, int, list[EndnoteChange]]:
		...
		if results:
			report = "\n".join(results)
			raise sach.InvalidInputException(f"Endnote error(s) found:\n{report}")
		...
		for file_path in self.spine_file_paths:
			...
			if needs_rewrite:
				with open(file_path, "w", encoding="utf-8") as file:
					file.write(sach.formatting.format_xhtml(dom.to_string()))
		...
		if processed == 0:
			raise sach.InvalidInputException("No files processed. Did you update the manifest and order the spine?")
		...
		if notes_changed > 0:
			...
			with open(self.endnotes_path, "w", encoding="utf-8") as file:
				file.write(sach.formatting.format_xhtml(endnotes_dom.to_string()))
	...
	def generate_onix(self, metadata_dom: EasyXmlTree | None = None) -> EasyXmlTree:
		...
		with importlib.resources.as_file(importlib.resources.files("sach.data").joinpath("opf2onix.xsl")) as opf2onix_xsl_filename:
	...
	def __process_noteref_link(self, change_list: list[EndnoteChange], current_note_number: int, file_name: str, link: EasyXmlElement, needs_rewrite: bool, notes_changed: int) -> tuple[bool, int]:
		...
		if not matches:
			raise sach.InvalidInputException(f"Couldn’t find endnote with anchor [val]{old_anchor}[/].")
		if len(matches) > 1:
			raise sach.InvalidInputException(f"Duplicate anchors in endnotes file for anchor [val]{old_anchor}[/].")
			...
			if dom.xpath("/html/body[contains(@epub:type, 'bodymatter') and count(./*[name() = 'article' or name() = 'section']) > 1 and count(./*[name() != 'article' and name() != 'section']) = 0]"):
				...
				if dom.xpath("/html/body[count(./*[name() = 'article' or name() = 'section']) = count(./*[(name() = 'article' or name() = 'section') and @id and attribute::*[re:test(local-name(), '^data-css-break-(before|after)$')]])]"):
					...
					for article_node in dom.xpath("/html/body/*[name() = 'article' or name() = 'section']"):
						...
						title = sach.formatting.generate_title(new_dom)
						...
						sach.formatting.format_xml_file(file_path.parent / new_filename)
					...
					if delete_original_file:
						...
						for new_file in new_files:
							...
							li_node.append(EasyXmlElement(f"""<a href="text/{new_file.filename}">{sach.formatting.escape_xml(new_file.title)}</a>"""))
					...
					sach.formatting.format_xml_file(self.toc_path)
					...
					sach.formatting.format_xml_file(self.metadata_file_path)
