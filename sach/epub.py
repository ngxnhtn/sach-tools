...
from sach.formatting import generate_epoch_timestamp
import sach
...
def convert_toc_to_ncx(epub_root_absolute_path: Path, toc_filename: str, xsl_filename: Path) -> sach.easy_xml.EasyXmlTree:
	"""
	...
	OUTPUTS
	An sach.easy_xml.EasyXmlTree representing the HTML5 ToC file
	"""
	...
	toc_tree = sach.easy_xml.EasyXmlTree(xhtml)
	with open(xsl_filename, "rb") as file:
		transform = etree.XSLT(etree.parse(file))
	ncx_dom = sach.easy_xml.EasyXmlTree(transform(etree.fromstring(xhtml), cwd=f"'{epub_root_absolute_path.as_posix()}/'"))
...
def write_epub(epub_root_absolute_path: Path, output_absolute_path: Path, last_update_datetime: datetime | None) -> None:
	...
	if sys.platform == "win32":
		sach.convert_directory_line_endings(epub_root_absolute_path, False)
