#!/usr/bin/env python3
"""
Contains the `LintMessage` class and the `SachEpub.Lint()` function, which is broken out of the `SachEpub` class for readability and maintainability.

Strictly speaking, `SachEpub.Lint()` should be a class member of `SachEpub`. But the function is very big and it makes editing easier to put in a separate file.
"""
...
if TYPE_CHECKING:
	from sach.sach_epub import SachEpub # Import under type checking guard to prevent circular import error.
...
See the `sach.print_error()` function for a comprehensive list of allowed codes.
...
"f-017", f"[path][link=file://{self.path / 'images/cover.jpg'}]cover.jpg[/][/] must be exactly {sach.COVER_WIDTH} × {sach.COVER_HEIGHT}."
...
"m-017", "Illegal [xml]<!\\[CDATA\\[[/]. [hint]Hint: Run [command]sach[/] [subcommand]clean[/] to canonicalize [xml]<!\\[CDATA\\[[/] elements.[/hint]"
...
class LintMessage:
	...
	def __init__(self, code: str, text: str, message_type:int=sach.MESSAGE_TYPE_WARNING, filename: Path | None = None, submessages: list[str] | set[str] | list[LintSubmessage] | None = None):
		self.code = code
...
def _build_section_tree(self: 'SachEpub') -> list[EbookSection]:
	"""
...
def _lint_metadata_checks(self: 'SachEpub') -> list[LintMessage]:
	"""
	...
	try:
		...
		for node in metadata_dom_with_parsed_long_description.xpath("/package/metadata/dc:description"):
			# If the HTML is malformed, this will emit `sach.InvalidXmlException` which we catch below.
		...
		if not regex.search(r"^\s*<p>", long_description) and not regex.match(r"^[A-Z_]+$", long_description.strip()):
			messages.append(LintMessage("m-016", "[xml]<dc:description[/] must contain an escaped XHTML fragment beginning with [xhtml]<p>[/].", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_node_tags([long_description_node])))
		...
		if nodes:
			messages.append(LintMessage("m-014", "Non-typogrified character in [xml]<dc:description>[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("m-078", "Nobel Prize strings must read [text]Nobel Prize in ...[/].", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("m-057", "Illegal [attr]@xml:lang[/] attribute in [xml]<dc:description>[/] element. [hint]Hint: [attr]@xml:lang[/] should be [attr]@lang[/].[/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))

		# `US` -> `U.S.`.
		nodes = metadata_dom_with_parsed_long_description.xpath("/package/metadata/dc:description/*[re:test(., '\\bUS\\b')]")
		if nodes:
			messages.append(LintMessage("t-047", "[text]US[/] set without periods. [hint]Hint: Use [text]U.S.[/][/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
		...
		if matches:
			messages.append(LintMessage("m-044", "Possessive [text]’[/] or [text]’s[/] outside of [xhtml]<a>[/] element in [xml]<dc:description>[/].", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(m, long_description_node.sourceline) for m in matches]))

		# Check for HTML entities in `<dc:description>`, but allow `&amp;amp`;.
		matches = regex.findall(r"&[a-z0-9]+?;", long_description.replace("&amp;", ""))
		if matches:
			messages.append(LintMessage("m-018", "HTML entities found. [hint]Hint: Use Unicode equivalents instead.[/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(m, long_description_node.sourceline) for m in matches]))

		nodes = metadata_dom_with_parsed_long_description.xpath("/package/metadata/dc:description/p[re:test(., '\\s[a-z]+ly-[a-z]')]")
		if nodes:
			messages.append(LintMessage("t-078", "Illegal hyphenated two-word phrasal adjective that begins with an adverb ending in [text]ly[/].", sach.MESSAGE_TYPE_WARNING, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))

	except sach.InvalidXmlException as ex:
		if long_description_node:
			messages.append(LintMessage("m-015", "Contents of [xml]<dc:description>[/] are not valid XHTML.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(f"{ex}", long_description_node.sourceline)]))
	...
	try:
		...
		if matches:
			messages.append(LintMessage("m-012", "Non-typogrified character in [xml]<dc:title>[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(m, title_node.sourceline) for m in matches]))
		...
		if matches and not self.metadata_dom.xpath("/package/metadata/meta[@property='dcterms:alternative']"):
			messages.append(LintMessage("m-052", "[xml]<dc:title>[/] element contains numbers, but no [xml]<meta property=\"dcterms:alternative\" refines=\"#title\"> element in metadata.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(m, title_node.sourceline) for m in matches]))
	except Exception:
	...
	try:
		...
		if matches:
			messages.append(LintMessage("m-050", "Non-typogrified character in [xml]<meta property=\"file-as\" refines=\"#title\">[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(m, file_as_node.sourceline) for m in matches]))
	except Exception:
	...
	try:
		...
		if matches:
			messages.append(LintMessage("m-013", "Non-typogrified character in [xml]<meta property=\"schema:abstract\">[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(m, description_node.sourceline) for m in matches]))
	except Exception:
	...
	if matching_nodes:
		messages.append(LintMessage("t-002", "Comma or period outside of double quote. [hint]Hint: Generally punctuation goes within single and double quotes.[/hint]", sach.MESSAGE_TYPE_WARNING, self.metadata_file_path, LintSubmessage.from_nodes(matching_nodes)))
	...
	if self.is_se_ebook:
		...
		if word_count_elements:
			...
			if word_count != "WORD_COUNT" and int(word_count) != self.get_word_count():
				messages.append(LintMessage("m-065", "Word count in metadata doesn’t match actual word count.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))
		else:
	...
	if production_notes_path.exists():
		with open(production_notes_path, "r", encoding="utf-8") as file:
			...
			if not first_char:
				messages.append(LintMessage("m-084", "Empty file. [hint]Hint: Empty files should be deleted.[/hint]", sach.MESSAGE_TYPE_ERROR, production_notes_path))
	...
	if nodes:
		messages.append(LintMessage("m-011", "Subtitle in metadata, but no expanded title element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	for implied_tag, tags in implied_tags.items():
		if self.metadata_dom.xpath(f"/package/metadata/meta[@property='schema:genre' and text()={sach.easy_xml.escape_xpath(implied_tag)}]"):
			for tag in tags:
				implying_nodes = self.metadata_dom.xpath(f"/package/metadata/meta[@property='schema:genre' and text()={sach.easy_xml.escape_xpath(tag)}]")
				for node in implying_nodes:
					submessages.append(LintSubmessage(f"{node.text} implies {implied_tag}", node.sourceline))

	if submessages:
		messages.append(LintMessage("m-058", "[val]schema:genre[/] that implies other [val]schema:genre[/]. [hint]Hint: Remove the [val]schema:genre[/] that is implied.[/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, submessages))

	# Check for `comprised of`.
	nodes = self.metadata_dom.xpath("/package/metadata/*[re:test(., '[Cc]omprised of')]")
	if nodes:
		messages.append(LintMessage("m-069", "[text]comprised of[/] in metadata. [hint]Hint: Is there a better phrase to use here?[/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))

	# Check for illegal em dashes in `<dc:subject>`.
	nodes = self.metadata_dom.xpath("/package/metadata/dc:subject[contains(text(), '—')]")
	if nodes:
		messages.append(LintMessage("m-019", "Illegal em dash in [xml]<dc:subject>[/] element. [hint]Hint: Use [text]--[/].[/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(node.text, node.sourceline) for node in nodes]))
	...
	if nodes:
		messages.append(LintMessage("m-096", "Whitespace around [text]--[/] in LCSH [xml]<dc:subject>[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("m-073", "Anonymous contributor values must be exactly [text]Anonymous[/].", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))

	# Check for metadata elements.
	nodes = self.metadata_dom.xpath("/package/metadata/*[not(name()='link') and not(normalize-space(.)) and not(./*)]")
	if nodes:
		messages.append(LintMessage("m-022", "Empty element in metadata.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("m-009", "Unexpected value for [attr]@href[/] attribute in [xml]<link rel=\"schema:codeRepository\">[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(f"Found: {node.get_attr('href')}\nExpected: {expected_vcs_url}", node.sourceline) for node in nodes]))
	...
	if nodes:
		...
		for node in nodes:
			...
			if node_inner_text not in SACH_GENRES and node_inner_text != "TAG":
				illegal_subjects.append(node)

		if illegal_subjects:
			messages.append(LintMessage("m-020", "Illegal value for [xml]<meta property=\"schema:genre\">[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_node_text(illegal_subjects)))

		if sorted(node_string_values) != node_string_values:
			messages.append(LintMessage("m-053", "[xml]<meta property=\"schema:genre\">[/] elements not in alphabetical order.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))

	elif self.is_se_ebook:
		messages.append(LintMessage("m-021", "No [xml]<meta property=\"schema:genre\">[/] element found.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))
	...
	if titles_missing_file_as:
		messages.append(LintMessage("m-062", "[xml]<dc:title>[/] element missing matching [xml]<meta property=\"file-as\">[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(titles_missing_file_as)))

	if titles_missing_title_type:
		messages.append(LintMessage("m-068", "[xml]<dc:title>[/] element missing matching [xml]<meta property=\"title-type\">[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(titles_missing_title_type)))
	...
	if matches:
		messages.append(LintMessage("m-017", "Illegal [xml]<!\\[CDATA\\[[/]. [hint]Hint: Run [command]sach[/] [subcommand]clean[/] to canonicalize [xml]<!\\[CDATA\\[[/] elements.[/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_matches(matches)))
	...
	if self.is_se_ebook:
		try:
			...
			if identifier.text != expected_identifier and not regex.search(r"_et-al$", identifier.text):
				messages.append(LintMessage("m-023", "Unexpected value for [xml]<dc:identifier>[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage(f"Found: {identifier.text}\nExpected: {expected_identifier}", identifier.sourceline)]))
		except Exception:
	...
	for node in nodes:
		try:
			...
			try:
				name = self.metadata_dom.xpath(f"/package/metadata/*[@id={sach.easy_xml.escape_xpath(refines)}]")[0].text
				if name == node.text:
	...
	if duplicate_names:
		messages.append(LintMessage("m-024", "[xml]<meta property=\"schema:alternateName\">[/] property identical to regular name. [hint]Hint: If the two are identical, then the full name [xml]<meta>[/] element must be removed.[/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, duplicate_names))

	if invalid_refines:
		messages.append(LintMessage("m-010", "Invalid [attr]@refines[/] attribute value.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, invalid_refines))
	...
	if nodes:
		messages.append(LintMessage("m-008", "Non-canonical Library of Congress Name Authority URI. Expected: [url]https://id.loc.gov/authorities/names/<IDENTIFIER>.html[/].", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	if invalid_description:
		messages.append(LintMessage("m-055", "[xml]<meta property=\"schema:abstract\">[/] element doesn’t end with a period.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, [LintSubmessage("..." + invalid_description[0].text[-10:], invalid_description[0].sourceline)]))
	...
	try:
		...
		if manifest.to_string().replace("\t", "") != self.generate_manifest().to_string().replace("\t", ""):
			messages.append(LintMessage("m-042", "[xml]<manifest>[/] element doesn’t match expected structure.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))
	except Exception:
		missing_metadata_elements.append("<manifest>")

	if missing_metadata_elements:
		messages.append(LintMessage("m-051", "Missing expected element in metadata.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, missing_metadata_elements))
	...
	for node in self.metadata_dom.xpath("/package/metadata/meta[@property=\"schema:abstract\"]") + self.metadata_dom.xpath("/package/metadata/dc:description"):
		...
		if matches:
			messages.append(LintMessage("y-001", "Possible typo: Doubled [text]a/the/and/of/or/as/if[/].", sach.MESSAGE_TYPE_WARNING, self.metadata_file_path, [LintSubmessage(m, node.sourceline) for m in matches]))

	nodes = self.metadata_dom.xpath("/package/metadata//*[re:test(., '[,;][a-z]', 'i')]")
	if nodes:
		messages.append(LintMessage("y-002", "Possible typo: Punctuation followed directly by a letter, without a space.", sach.MESSAGE_TYPE_WARNING, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("m-083", "[xhtml]<meta property=\"title-type\">[/] element without sibling element with contents of [val]main[/], [val]subtitle[/], [val]expanded[/], or [val]short[/].", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))

	nodes = self.metadata_dom.xpath("/package/metadata/meta[@property='term' and re:test(., '^https?://')]")
	if nodes:
		messages.append(LintMessage("m-066", "Subject identifiers must be IDs and not URLs.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	if non_alphabetized_marc_relator_groups:
		messages.append(LintMessage("m-087", "MARC relators not in alphabetical order.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(non_alphabetized_marc_relator_groups)))

	if invalid_marc_relators_nodes:
		messages.append(LintMessage("m-091", "Possibly misspelled MARC relator found.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_node_text(invalid_marc_relators_nodes)))
	...
	for _, v in same_as_entries.items():
		...
		if invalid_nodes:
			messages.append(LintMessage("m-093", "A single contributor has multiple [val]schema:sameAs[/] URLs to the same site.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(invalid_nodes)))
...
def _get_malformed_urls(dom: EasyXmlTree, filename: Path) -> list[LintMessage]:
	...
	if nodes:
		messages.append(LintMessage("m-003", "Bare URL without trailing slash.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("m-001", "Non-canonical Faded Page URL. Expected: [url]https://www.fadedpage.com/showbook.php?pid=<BOOK-ID>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	search_regex = r"^https?://(.+\.)?gutenberg\.net\.au/"
	expected_regex = r"^https://www\.gutenberg\.net\.au(/|/.+\.html)$"
	nodes = dom.xpath(f"/package/metadata/*[( (re:test(., '{search_regex}') and not(re:test(., '{expected_regex}'))) or (re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))) )] | /html/body//a[re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))]")
	if nodes:
		messages.append(LintMessage("m-076", "Non-canonical Project Gutenberg Australia URL. Expected: [url]https://www.gutenberg.net.au/<PATH>/<FILENAME>.html[/] or [url]https://www.gutenberg.net.au/[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	search_regex = r"https?://books\.google\.com/"
	expected_regex = r"^https?://books\.google\.com/books\?id=[^/\?#&]+?$"
	nodes = dom.xpath(f"/package/metadata/*[( (re:test(., '{search_regex}') and not(re:test(., '{expected_regex}'))) or (re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))) )] | /html/body//a[re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))]")
	if nodes:
		messages.append(LintMessage("m-004", "Non-canonical Google Books URL. Expected: [url]https://books.google.com/books?id=<BOOK-ID>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	search_regex = r"https?://www\.google\.com/books/"
	expected_regex = r"^https?://www\.google\.com/books/edition/[^/]+/[^/\?#&]+$"
	nodes = dom.xpath(f"/package/metadata/*[( (re:test(., '{search_regex}') and not(re:test(., '{expected_regex}'))) or (re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))) )] | /html/body//a[re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))]")
	if nodes:
		messages.append(LintMessage("m-060", "Non-canonical Google Books URL. Expected: [url]https://www.google.com/books/edition/<BOOK-NAME>/<BOOK-ID>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	search_regex = r"^https?://((.+\.)?hathitrust\.org|hdl\.handle\.net)"
	expected_regex = r"^https://catalog\.hathitrust\.org/Record/[^/]+$"
	nodes = dom.xpath(f"/package/metadata/*[( (re:test(., '{search_regex}') and not(re:test(., '{expected_regex}'))) or (re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))) )] | /html/body//a[re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))]")
	if nodes:
		messages.append(LintMessage("m-005", "Non-canonical HathiTrust URL. Expected: [url]https://catalog.hathitrust.org/Record/<BOOK-ID>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	search_regex = r"^https?://(.+\.)?gutenberg\.org/"
	expected_regex = r"^https://www\.gutenberg\.org(/|/ebooks/[0-9]+)$"
	nodes = dom.xpath(f"/package/metadata/*[( (re:test(., '{search_regex}') and not(re:test(., '{expected_regex}'))) or (re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))) )] | /html/body//a[re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))]")
	if nodes:
		messages.append(LintMessage("m-006", "Non-canonical Project Gutenberg URL. Expected: [url]https://www.gutenberg.org/ebooks/<BOOK-ID>[/] or [url]https://www.gutenberg.org/[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("m-007", "Non-canonical Internet Archive URL. Expected: [url]https://archive.org/details/<BOOK-ID>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	search_regex = r"^https?://web\.archive\.org/"
	expected_regex = r"^https://web\.archive\.org/web/[0-9]+/.+$"
	nodes = dom.xpath(f"/package/metadata/*[( (re:test(., '{search_regex}') and not(re:test(., '{expected_regex}'))) or (re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))) )] | /html/body//a[re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))]")
	if nodes:
		messages.append(LintMessage("m-043", "Non-canonical Wayback Machine URL. Expected: [url]https://web.archive.org/web/<DATE>/<ARCHIVED-URL>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	nodes = dom.xpath(f"/package/metadata/*[( (re:test(., '{search_regex}') and not(re:test(., '{expected_regex}'))) or (re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))) )] | /html/body//a[re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))]")
	if nodes:
		messages.append(LintMessage("m-002", "Non-canonical Project Gutenberg Canada URL. Expected: [url]https://gutenberg.ca/<PATH>/<FILENAME>.html[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	nodes = dom.xpath(f"/package/metadata/*[( (re:test(., '{search_regex}') and not(re:test(., '{expected_regex}'))) or (re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))) )] | /html/body//a[re:test(@href, '{search_regex}') and not(re:test(@href, '{expected_regex}'))]")
	if nodes:
		messages.append(LintMessage("m-085", "Non-canonical PGDP URL. Expected: [url]https://www.pgdp.net/[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	return messages

def _get_selectors_and_rules(self: 'SachEpub') -> tuple[dict[str, str], list[str]]:
	"""
	Helper function used in `self.lint()`.
...
def files_not_in_spine(self: 'SachEpub') -> set[Path]:
	"""
...
def _lint_css_checks(self: 'SachEpub', local_css_path: Path, abbr_with_whitespace: list[str]) -> list[LintMessage]:
	"""
	...
	if matches and "@namespace xml \"http://www.w3.org/XML/1998/namespace\";" not in self.local_css:
		messages.append(LintMessage("c-003", "[css-selector]\\[xml|attr][/] selector in CSS, but no XML namespace declared. [hint]Hint: Add [css]@namespace xml \"http://www.w3.org/XML/1998/namespace\";[/] to the top of this CSS file.[/hint]", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css-selector")))
	...
	if abbr_with_whitespace:
		...
		messages.append(LintMessage("c-005", "Illegal [css]white-space: nowrap;[/] applied to [css-selector]abbr[/] selector.", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css-selector")))

	matches = source_file.findall(r"text-align:\s*left\s*;")
	if matches:
		messages.append(LintMessage("c-016", "Illegal [css]text-align: left;[/]. [hint]Hint: use [css]text-align: initial;[/] instead.[/hint]", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css")))

	matches = source_file.findall(r"[a-z\-]+\s*:\s*[0-9\.]\s?rem;")
	if matches:
		messages.append(LintMessage("c-022", "Illegal [val]rem[/] unit. [hint]Hint: use [val]em[/] instead.[/hint]", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css")))

	matches = source_file.findall(r"font-size\s*:\s*[0-9\.]+(?![0-9\.]|em|ex)")
	if matches:
		messages.append(LintMessage("c-023", "Illegal unit used to set [css-property]font-size[/]. [hint]Hint: Use [val]em[/] units.[/hint]", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css")))

	matches = source_file.findall(r"line-height\s*:\s*[0-9\.]+(?!;|[0-9\.]+)")
	if matches:
		messages.append(LintMessage("c-024", "Illegal unit used to set [css-property]line-height[/]. [hint]Hint: [css-property]line-height[/] is set without any units.[/hint]", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css")))
	...
	if matches:
		messages.append(LintMessage("c-025", "Illegal percent unit used to set [css-property]height[/] or positioning property. [hint]Hint: Use [val]vh[/] to specify vertical-oriented properties like [css-property]height[/] or [css-property]position[/].[/hint]", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css")))

	matches = source_file.findall(r"font-size: 0?\.[0-9]+")
	if matches:
		messages.append(LintMessage("c-027", "Illegal [css-property]font-size[/] below [val]1[/].", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css")))
...
def _lint_image_checks(self: 'SachEpub', filename: Path) -> list[LintMessage]:
	"""
	...
	if filename.suffix == ".jpeg":
		messages.append(LintMessage("f-011", "JPEG files must end in [path].jpg[/].", sach.MESSAGE_TYPE_ERROR, filename))
	elif filename.suffix == ".tiff":
		messages.append(LintMessage("f-012", "TIFF files must end in [path].tif[/].", sach.MESSAGE_TYPE_ERROR, filename))
	...
	if "cover.source" not in filename.name:
		try:
			...
			raise sach.InvalidFileException(f"Couldn’t identify image type of [path][link=file://{filename}]{filename.name}[/][/].") from ex

		# Check the source cover image.
		if self.path / "images" / "cover.jpg" == filename:
			if image.size != (sach.COVER_WIDTH, sach.COVER_HEIGHT):
				messages.append(LintMessage("f-017", f"[path][link=file://{self.path / 'images/cover.jpg'}]cover.jpg[/][/] must be exactly {sach.COVER_WIDTH} × {sach.COVER_HEIGHT}.", sach.MESSAGE_TYPE_ERROR, filename))
		...
		if filename.is_relative_to(self.content_path / "images") or (self.path / "images" / "cover.jpg" == filename):
			if os.path.getsize(filename) > 1500000: # 1.5MB.
				messages.append(LintMessage("f-016", "Image more than 1.5MB in size.", sach.MESSAGE_TYPE_ERROR, filename))
			...
			if image.size[0] * image.size[1] > 4000000:
				messages.append(LintMessage("f-018", "Image greater than 4,000,000 pixels square in dimension.", sach.MESSAGE_TYPE_ERROR, filename))

			if filename.suffix == ".png" and not sach.images.has_transparency(image):
				messages.append(LintMessage("f-019", "[path].png[/] file without transparency. [hint]Hint: If an image doesn’t have transparency, it should be saved as a [path].jpg[/].[/hint]", sach.MESSAGE_TYPE_ERROR, filename))

	return messages

def _lint_svg_checks(self: 'SachEpub', source_file: SourceFile, svg_dom: EasyXmlTree, root: str) -> list[LintMessage]:
	"""
	...
	if f"{os.sep}src{os.sep}images{os.sep}" not in root:
		if self.cover_path and filename.name == self.cover_path.name:
			...
			if nodes:
				messages.append(LintMessage("s-002", "Lowercase letters in cover. [hint]Hint: Cover text must be all uppercase.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
		...
		if filename.name == "titlepage.svg":
			...
			if nodes:
				messages.append(LintMessage("s-003", "Lowercase letters in titlepage. [hint]Hint: Titlepage text must be all uppercase except [text]translated by[/] and [text]illustrated by[/].[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if self.cover_path and filename.name == self.cover_path.name:
		...
		if svg_dom.xpath("//image[re:test(@xlink:href, '^.{299}A{50,}')]"):
			messages.append(LintMessage("m-063", "Cover image has not been built.", sach.MESSAGE_TYPE_ERROR, filename))
	...
	try:
		...
		try:
			...
			if "." in svg_dimensions[2] or "." in svg_dimensions[3]:
				messages.append(LintMessage("x-022", "Illegal fractions in SVG [attr]@viewBox[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename))
		except Exception as ex:
			raise sach.InvalidFileException(f"Couldn’t parse SVG [attr]@viewBox[/] attribute in [path][link=file://{filename.resolve()}]{filename}[/][/].") from ex
	except IndexError:
		# No viewbox.
		pass
	...
	if nodes:
		...
		if invalid_transform_attributes:
			messages.append(LintMessage("x-003", "Illegal [attr]@transform[/] attribute. [hint]Hint: SVGs should be optimized to remove use of [attr]@transform[/]. Try using Inkscape to save as an “optimized SVG”.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, invalid_transform_attributes))

		if invalid_id_attributes:
			messages.append(LintMessage("x-014", "Illegal [attr]@id[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, invalid_id_attributes))
	...
	if nodes:
		messages.append(LintMessage("x-004", "Illegal [xml]style=\"fill: #000\"[/] or [xml]fill=\"#000\"[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if filename.name != "logo.svg": # Do as I say, not as I do...
		...
		if nodes:
			messages.append(LintMessage("x-005", "Illegal [attr]@height[/] or [attr]@width[/] attribute on root [xml]<svg>[/] element. [hint]Hint: Size SVGs using the [attr]@viewBox[/] attribute only.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	nodes = svg_dom.xpath("/svg[not(@viewBox)]")
	if nodes:
		messages.append(LintMessage("x-006", "SVG root without [attr]@viewBox[/] attribute. [hint]Hint: [attr]@viewBox[/] must be correctly capitalized.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	return messages

def _lint_special_file_checks(self: 'SachEpub', source_file: SourceFile, dom: EasyXmlTree, ebook_flags: dict[str, bool], special_file: str) -> list[LintMessage]:
	"""
  ...
  self
	...
	if self.is_se_ebook and special_file in ("colophon", "imprint"):
		...
		if nodes:
			messages.append(LintMessage("m-041", "Hathi Trust link text must be exactly [text]HathiTrust Digital Library[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

		nodes = dom.xpath("/html/body//a[re:test(@href, '^https://www\\.pgdp\\.net') and text()!='Distributed Proofreaders']")
		if nodes:
			messages.append(LintMessage("m-071", "DP link must be exactly [text]Distributed Proofreaders[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

		nodes = dom.xpath("/html/body//a[re:test(@href, '^https://www\\.pgdp\\.org/ols\\b') and text()!='Distributed Proofreaders Open Library System']")
		if nodes:
			messages.append(LintMessage("m-072", "DP OLS link must be exactly [text]Distributed Proofreaders Open Library System[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

		nodes = dom.xpath("/html/body//a[re:test(@href, '^https://www\\.pgdpcanada\\.net') and text()!='Distributed Proofreaders Canada']")
		if nodes:
			messages.append(LintMessage("m-080", "DP link must be exactly [text]Distributed Proofreaders Canada[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

		nodes = dom.xpath("/html/body//a[re:test(@href, '^https://www\\.fadedpage\\.com') and not(text()='Faded Page')]")
		if nodes:
			messages.append(LintMessage("m-082", "Faded Page link text must be exactly [text]Faded Page[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

		nodes = dom.xpath("/html/body//p[re:test(., '\\ba transcription\\b') and ./a[contains(@href, '#transcriptions')]]")
		if nodes:
			messages.append(LintMessage("t-071", "Multiple transcriptions listed, but preceding text is [text]a transcription[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if self.is_se_ebook and special_file == "colophon":
		if self.metadata_dom.xpath("/package/metadata/meta[@property='role' and text()='trl']") and "translated from" not in source_file.contents:
			messages.append(LintMessage("m-025", "Translator found in metadata, but no [text]translated from LANG[/] block in colophon.", sach.MESSAGE_TYPE_ERROR, filename))
		...
		if missing_colophon_vars:
			messages.append(LintMessage("m-036", "Variable not replaced with value.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_matches(missing_colophon_vars)))
		...
		if missing_source_links:
			messages.append(LintMessage("m-037", "Expected transcription/page scan source link not found.", sach.MESSAGE_TYPE_ERROR, filename, [LintSubmessage(f"Expected: {text}") for text in missing_source_links]))
		...
		for node in dom.xpath("/html/body//a[re:test(@href, '(gutenberg\\.org/ebooks/[0-9]+|hathitrust\\.org|/archive\\.org|books\\.google\\.com|www\\.google\\.com/books/)')]"):
			if not self.metadata_dom.xpath(f"/package/metadata/dc:source[contains(text(), {sach.easy_xml.escape_xpath(node.get_attr('href'))})]"):
				missing_page_scan_links.append(node)

		if missing_page_scan_links:
			messages.append(LintMessage("m-059", "Link found in colophon, but missing matching [xhtml]<dc:source>[/] element in metadata.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(missing_page_scan_links)))

		if ebook_flags["has_multiple_transcriptions"] and not dom.xpath("/html/body//a[contains(@href, '#transcriptions')]"):
			messages.append(LintMessage("m-074", "Multiple transcriptions found in metadata, but no link to [text]EBOOK_URL#transcriptions[/].", sach.MESSAGE_TYPE_ERROR, filename))

		if ebook_flags["has_multiple_page_scans"] and not dom.xpath("/html/body//a[contains(@href, '#page-scans')]"):
			messages.append(LintMessage("m-075", "Multiple page scans found in metadata, but no link to [text]EBOOK_URL#page-scans[/].", sach.MESSAGE_TYPE_ERROR, filename))
		...
		if nodes:
			messages.append(LintMessage("s-105", "Date without parent [xhtml]<time>[/] element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

		# A range of years for published should be `published between X and Y`.
		nodes = dom.xpath(f"/html/body//p[re:test(., '\\b(published|completed) (in|between|circa) [0-9]+{sach.WORD_JOINER}?–{sach.WORD_JOINER}?[0-9]+\\b')]")
		if nodes:
			messages.append(LintMessage("m-081", "When a work was published or completed between a range of years, the text must be [text]between year1 and year2[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("s-016", "Incorrect [text]the[/] before Google Books link.", sach.MESSAGE_TYPE_ERROR, filename, [LintSubmessage("the<br/>\n" + node.to_string(), node.sourceline) for node in nodes]))
		...
		if nodes:
			messages.append(LintMessage("s-053", "Colophon line not preceded by [xhtml]<br/>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

		nodes = dom.xpath("/html/body//b[re:test(., 'anonymous', 'i') and re:test(@epub:type, 'z3998:.*?name')]")
		if nodes:
			messages.append(LintMessage("s-092", "Anonymous contributor with [val]z3998:*-name[/] semantic.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("s-100", "Anonymous digital contributor value not exactly [text]An Anonymous Volunteer[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("s-101", "Anonymous primary contributor value not exactly [text]Anonymous[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("t-006", "Comma after producer name, but there are only two producers.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
		...
		if ebook_flags["has_multiple_transcriptions"] or ebook_flags["has_multiple_page_scans"]:
			...
			if nodes:
				messages.append(LintMessage("t-072", "[text]various sources[/] link not preceded by [text]from[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("s-106", "Proper name in the colophon without parent [xhtml]<a href=\"...\">[/] or [xhtml]<b epub:type=\"z3998:given-name\">[/], or [xhtml]<b>[/] if anonymous.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("s-107", "Anonymous contributors in the colophon must be exactly [xhtml]<b>An Anonymous Volunteer</b>[/] or [xhtml]<b>An Unknown Artist</b>[/] or [xhtml]<b>An Anonymous Sponsor</b>[/]. [hint]Hint: Is there a missing [attr]@epub:type[/] semantic?[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	# We don't have a standard yet for more than two sources (transcription and scan) so just ignore that case for now.
		...
		if nodes:
			messages.append(LintMessage("s-016", "Incorrect [text]the[/] before Google Books link.", sach.MESSAGE_TYPE_ERROR, filename, [LintSubmessage("the " + node.to_string(), node.sourceline) for node in nodes]))

		# Check if we forgot to fill any variable slots.
		missing_imprint_vars = [match for var in SACH_VARIABLES if (match := source_file.search(fr"\b{var}\b")) is not None]
		if missing_imprint_vars:
			messages.append(LintMessage("m-036", "Variable not replaced with value.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_matches(missing_imprint_vars)))
		...
		if ebook_flags["has_multiple_transcriptions"] or ebook_flags["has_multiple_page_scans"]:
			...
			if nodes:
				messages.append(LintMessage("t-072", "[text]various sources[/] link not preceded by [text]from[/].", sach.MESSAGE_TYPE_ERROR, filename))
	...
	# Endnote checks.
		...
		if nodes:
			messages.append(LintMessage("s-039", "[text]Ibid[/] in endnotes. [hint]Hint: “Ibid” means “The previous reference” which is meaningless with popup endnotes, and must be replaced by the actual thing [text]Ibid[/] refers to, unless it refers to text within the same endnote.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("s-056", "Last [xhtml]<p>[/] child of endnote missing backlink.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
		...
		if nodes:
			messages.append(LintMessage("s-057", "Backlink noteref fragment identifier doesn’t match endnote number.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
		...
		nodes = dom.xpath(f"/html/body//section[re:test(@epub:type, '\\bendnotes\\b')]/ol/li/p[last()][re:test(., '\\.”?{sach.WORD_JOINER}?—[A-Z].{{0,20}}\\s*↩$')]")
		if nodes:
			messages.append(LintMessage("s-064", "Endnote citation not wrapped in [xhtml]<cite>[/]. [hint]Hint: Em dashes go within [xhtml]<cite>[/] and it are preceded by exactly one space.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
		...
		if nodes:
			messages.append(LintMessage("t-027", "Endnote backlink not preceded by exactly one space.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	# Check LoI descriptions to see if they match associated `<figcaption>s`.
		...
		for node in dom.xpath("/html/body/nav[contains(@epub:type, 'loi')]//li//a"):
			...
			file_dom = self.get_dom(sach.abspath_relative_to(Path(chapter_ref), filename))

			try:
				figure = file_dom.xpath(f"//*[@id={sach.easy_xml.escape_xpath(figure_ref)}]")[0]
			except Exception:
			...
			for child in figure.xpath(".//img|./figcaption"):
				...
				if child.tag == "img":
					...
					loi_text_to_compare = loi_text_to_compare.replace(sach.NO_BREAK_SPACE, ' ').replace(sach.WORD_JOINER, '')
		...
		if mismatched_loi_nodes:
			messages.append(LintMessage("s-041", "LoI entry text doesn’t match either the referenced element’s [xhtml]<figcaption>[/] element or its [xhtml]<img>[/] [attr]@alt[/] attribute.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(mismatched_loi_nodes)))
...
def _lint_xhtml_css_checks(source_file: SourceFile, dom: EasyXmlTree) -> list[LintMessage]:
	...
	if nodes:
		messages.append(LintMessage("c-004", "Illegal [css-property]border-color[/] specified on element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-010", "[xhtml]<footer>[/] missing [css]margin-top: 1em; text-align: VALUE;[/]. [hint]Hint: [css-property]text-align[/] is usually set to [val]right[/].[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-011", "Element with [css]text-align: center;[/] but [css-property]text-indent[/] is [val]1em[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-012", "Sectioning element without heading content, and without [css]margin-top: 20vh;[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-013", "Element with margin or padding not in increments of [val].5em[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-014", "[xhtml]<table>[/] element without explicit margins. [hint]Hint: Most tables need [css]margin: 1em;[/] or [css]margin: 1em auto 1em auto;[/].[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-015", "Element after or containing [val]z3998:salutation[/] semantic doesn’t have [css]text-indent: 0;[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-017", "Element with [val]z3998:postscript[/] semantic, but without [css]margin-top: 1em;[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-018", "Element with [val]z3998:postscript[/] semantic, but without [css]text-indent: 0;[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-019", "Element with [val]z3998:signature[/] semantic, but without [css]font-variant: small-caps;[/] or [css]font-style: italic;[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-020", "Multiple [xhtml]<article>[/]s or [xhtml]<section>[/]s in file, but missing [css]break-after: page;[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("c-026", "Table that appears to be listing numbers, but without [css]font-variant-numeric: tabular-nums;[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))

	return messages

def _lint_xhtml_metadata_checks(self: 'SachEpub', filename: Path, dom: EasyXmlTree) -> list[LintMessage]:
	"""
	...
	if not dom.xpath("/html/body//nav[contains(@epub:type, 'landmarks')]"):
		if dom.xpath("/html/body/*[contains(@epub:type, 'introduction') and not(@data-parent)]") and not self.metadata_dom.xpath("/package/metadata/meta[(@property='role') and text()='win']"):
			messages.append(LintMessage("m-030", "[val]introduction[/] semantic inflection found, but no MARC relator [val]win[/] (Writer of introduction).", sach.MESSAGE_TYPE_WARNING, filename))
		...
		if dom.xpath("/html/body/*[contains(@epub:type, 'preface') and not(@data-parent)]") and not self.metadata_dom.xpath("/package/metadata/meta[(@property='role') and text()='wpr']"):
			messages.append(LintMessage("m-031", "[val]preface[/] semantic inflection found, but no MARC relator [val]wpr[/] (Writer of preface).", sach.MESSAGE_TYPE_WARNING, filename))
		...
		if dom.xpath("/html/body/*[contains(@epub:type, 'afterword') and not(@data-parent)]") and not self.metadata_dom.xpath("/package/metadata/meta[(@property='role') and text()='waw']"):
			messages.append(LintMessage("m-032", "[val]afterword[/] semantic inflection found, but no MARC relator [val]waw[/] (Writer of afterword).", sach.MESSAGE_TYPE_WARNING, filename))
		...
		if dom.xpath("/html/body/*[contains(@epub:type, 'foreword') and not(@data-parent)]") and not self.metadata_dom.xpath("/package/metadata/meta[(@property='role') and text()='wfw']"):
			messages.append(LintMessage("m-086", "[val]foreword[/] semantic inflection found, but no MARC relator [val]wfw[/] (Writer of foreword).", sach.MESSAGE_TYPE_WARNING, filename))
		...
		if dom.xpath("/html/body/section[re:test(@epub:type, '\\bendnotes\\b') and not(@data-parent)]") and not self.metadata_dom.xpath("/package/metadata/meta[(@property='role') and text()='ann']"):
			messages.append(LintMessage("m-033", "[val]endnotes[/] semantic inflection found, but no MARC relator [val]ann[/] (Annotator).", sach.MESSAGE_TYPE_WARNING, filename))
		...
		if dom.xpath("/html/body/*[contains(@epub:type, 'loi') and not(@data-parent)]") and not self.metadata_dom.xpath("/package/metadata/meta[(@property='role') and (text()='ill' or text()='pht')]"):
			messages.append(LintMessage("m-034", "[val]loi[/] semantic inflection found, but no MARC relator [val]ill[/] (Illustrator) or [val]pht[/] (Photographer).", sach.MESSAGE_TYPE_WARNING, filename))
		...
		if dom.xpath("/html/body/*[contains(@epub:type, 'dedication') and not(@data-parent)]") and not self.metadata_dom.xpath("/package/metadata/meta[(@property='role') and text()='dto']"):
			messages.append(LintMessage("m-090", "[val]dedication[/] semantic inflection found, but no MARC relator [val]dto[/] (Dedicator).", sach.MESSAGE_TYPE_WARNING, filename))
	...
	if nodes:
		messages.append(LintMessage("m-061", "Link must be preceded by [text]the[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	return messages

def _lint_xhtml_syntax_checks(self: 'SachEpub', source_file: SourceFile, dom: EasyXmlTree, ebook_flags: dict[str, bool], language: str, section_tree: list[EbookSection]) -> list[LintMessage]:
	"""
	...
	if matches:
		messages.append(LintMessage("s-001", "Illegal numeric entity.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_matches(matches)))
	...
	if nodes:
		...
		messages.append(LintMessage("s-005", "Nested [xhtml]<blockquote>[/] element.", sach.MESSAGE_TYPE_WARNING, filename, submessages))
	...
	if nodes:
		...
		messages.append(LintMessage("s-006", "Poem or verse [xhtml]<p>[/] (stanza) without [xhtml]<span>[/] (line) element.", sach.MESSAGE_TYPE_WARNING, filename, submessages))
	...
	if nodes:
		messages.append(LintMessage("s-007", "Element requires at least one block-level child.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-008", "[xhtml]<br/>[/] element found before closing tag of block-level element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for `<hgroup>` elements with only one child.
	nodes = dom.xpath("/html/body//hgroup[count(*)=1]")
	if nodes:
		messages.append(LintMessage("s-009", "[xhtml]<hgroup>[/] element with only one child.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-010", "Empty element. [hint]Hint: Use [xhtml]<hr/>[/] for thematic breaks if appropriate.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-011", "Element without [attr]@id[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for `<pre>` elements.
	nodes = dom.xpath("/html/body//pre")
	if nodes:
		messages.append(LintMessage("s-013", "Illegal [xhtml]<pre>[/] element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-014", "[xhtml]<br/>[/] after block-level element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-012", "Illegal [xhtml]<hr/>[/] as last child.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-015", "Element has [val]subtitle[/] semantic, but without a sibling having a [val]title[/] semantic.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-017", "Illegal [xhtml]<m:mfenced>[/]. [hint]Hint: Use [xhtml]<m:mrow><m:mo fence=\"true\">(</m:mo>...<m:mo fence=\"true\">)</m:mo></m:mrow>[/].[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for `<figure>` elements without `id` attributes.
	nodes = dom.xpath("/html/body//img[@id]")
	if nodes:
		messages.append(LintMessage("s-018", "[xhtml]<img>[/] element with [attr]@id[/] attribute. [hint]Hint: [attr]@id[/] attributes go on parent [xhtml]<figure>[/] elements. Images that are inline (i.e. that do not have a parent [xhtml]<figure>[/]) do not have [attr]@id[/] attributes.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-019", "[xhtml]<h#>[/] element with [attr]@id[/] attribute. [hint]Hint: [xhtml]<h#>[/] elements should be wrapped in [xhtml]<section>[/] elements, which should hold the [attr]@id[/] attribute.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))
	...
	#if nodes:
	#	messages.append(LintMessage("t-xxx", "Legal case without parent [xhtml]<i>[/].", sach.MESSAGE_TYPE_WARNING, filename, {f"{node.to_string()}." for node in nodes}))
	...
	if len(dom.xpath("/html/body/*[name()='section' or name()='article']/*[re:test(name(), '^h[1-6]$') or name()='hgroup'] | /html/body/*[name()='section' or name()='article']/header/*[re:test(name(), '^h[1-6]$') or name()='hgroup']"))==1:
		title = sach.formatting.generate_title(dom)

		unexpected_titles: list[LintSubmessage] = []
		for node in dom.xpath(f"/html/head/title[text()!={sach.easy_xml.escape_xpath(title.replace('&amp;', '&'))}]"):
			unexpected_titles.append(LintSubmessage(f"Found: {node.text}\nExpected: {title}", node.sourceline))

		if unexpected_titles:
			messages.append(LintMessage("s-021", "Unexpected value for [xhtml]<title>[/] element. [hint]Hint: Beware hidden Unicode characters![/hint]", sach.MESSAGE_TYPE_ERROR, filename, unexpected_titles))
	...
	for node in nodes:
		...
		if subtitle_matches:
			for title_header, subtitle, title_footer in subtitle_matches:
				...
				title = sach.formatting.remove_tags(title).strip()
	...
	if incorrectly_titlecased_titles:
		messages.append(LintMessage("s-023", "Title not correctly titlecased.", sach.MESSAGE_TYPE_WARNING, filename, incorrectly_titlecased_titles))
	...
	if nodes:
		messages.append(LintMessage("s-024", "Entirely non-English header set in italics. [hint]Hint: Don’t use italics, and put the [attr]@xml:lang[/] attribute on the [xhtml]<h#>[/] element.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-025", "Illegal scare quotes or ending punctuation in title of media. [hint]Hint: surrounding punctuation isn’t a part of a title.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-026", "Invalid Roman numeral.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if invalid_parent_ids:
		messages.append(LintMessage("s-029", "Section with [attr]@data-parent[/] attribute, but no section having that [attr]@id[/] in ebook.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(invalid_parent_ids)))
	if invalid_headers:
		messages.append(LintMessage("s-085", "[xhtml]<h#>[/] element found in a [xhtml]<section>[/] or a [xhtml]<article>[/] at an unexpected level. [hint]Hint: Headings not in the title page start at [xhtml]<h2>[/]. If this work has parts, should this header be [xhtml]<h3>[/] or higher?[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(invalid_headers)))
	...
	for node in dom.xpath("//*[@epub:type]"):
		...
		for val in regex.split(r"\s+", node.get_attr("epub:type")):
			...
			if val.startswith("z3998:"):
				...
				if bare_val not in SACH_SEMANTIC_VOCABULARY:
					incorrect_attrs.append(LintSubmessage(val, node.sourceline))
	...
	if duplicate_attrs:
		messages.append(LintMessage("s-031", "Duplicate value in [attr]@epub:type[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(duplicate_attrs)))
	if incorrect_attrs:
		messages.append(LintMessage("s-032", "Invalid value for [attr]@epub:type[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, incorrect_attrs))
	if unnecessary_z3998_attrs:
		messages.append(LintMessage("s-034", "Semantic used from the z3998 vocabulary, but the same semantic exists in the EPUB vocabulary.", sach.MESSAGE_TYPE_ERROR, filename, unnecessary_z3998_attrs))
	...
	if filename.name not in IGNORED_FILENAMES or filename.name == "loi.xhtml":
		...
		for node in nodes:
			...
			if file_language != language:
				messages.append(LintMessage("s-033", "[attr]@xml:lang[/] value on [xhtml]<html>[/] element doesn’t match language set in metadata.", sach.MESSAGE_TYPE_WARNING, filename, [LintSubmessage(f"Found: {file_language}\nExpected: {language}", node.sourceline)]))

	nodes = dom.xpath("/html/body//em[re:test(., '(^“[^”]+$|^[^“]+”$|^“.+”$)')]")
	if nodes:
		messages.append(LintMessage("s-030", "[xhtml]<em>[/] outside of quotation marks.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-035", "Endnote containing only [xhtml]<cite>[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-036", "No [val]frontmatter[/] semantic inflection for what looks like a frontmatter file.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-037", "No [val]backmatter[/] semantic inflection for what looks like a backmatter file.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-038", "Illegal asterism. [hint]Hint: Section/scene breaks must be defined by an [xhtml]<hr/>[/] element.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	nodes = dom.xpath("/html/body//*[@data-parent = parent::*/@id]")
	if nodes:
		messages.append(LintMessage("s-040", "Element with [attr]@data-parent[/] attribute, but its parent is in the same file.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for `<table>` element without a `<tbody>` child.
	nodes = dom.xpath("/html/body//table[not(tbody)]")
	if nodes:
		messages.append(LintMessage("s-042", "[xhtml]<table>[/] element without [xhtml]<tbody>[/] child.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-043", "[val]se:short-story[/] semantic on element that isn’t [xhtml]<article>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-044", "Element with poem or verse semantic, without descendant [xhtml]<p>[/] (stanza) element.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if filtered_nodes:
		messages.append(LintMessage("s-045", "[xhtml]<abbr>[/] element without semantic inflection.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(filtered_nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-047", "[val]noteref[/] as a direct child of element with poem or verse semantic. [hint]Hint: [val]noteref[/]s should be in their parent [xhtml]<span>[/].[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-048", "[val]se:name[/] semantic on block element. [hint]Hint: [val]se:name[/] indicates the contents is the name of something.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-049", "[xhtml]<header>[/] element whose only child is an [xhtml]<h#>[/] element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-050", "[xhtml]<span>[/] element appears to exist only to apply [attr]@epub:type[/]. [hint]Hint: [attr]@epub:type[/] should go on the parent element instead, without a [xhtml]<span>[/] element.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for language tags misusing the `unk` lang code.
	nodes = dom.xpath("//*[@xml:lang='unk']")
	if nodes:
		messages.append(LintMessage("s-051", "Illegal [val]unk[/] value for language. [hint]Hint: Did you mean [val]und[/] (undefined)?[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for `title` attributes on `<abbr>` elements.
	nodes = dom.xpath("/html/body//abbr[@title]")
	if nodes:
		messages.append(LintMessage("s-052", "[xhtml]<abbr>[/] element with illegal [attr]@title[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-054", "[xhtml]<cite>[/] as child of [xhtml]<p>[/] in [xhtml]<blockquote>[/]. [hint]Hint: [xhtml]<cite>[/] should be the direct child of [xhtml]<blockquote>[/].[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-055", "[xhtml]<th>[/] element not in [xhtml]<thead>[/] ancestor. [hint]Hint: [xhtml]<th>[/] elements used as mid-table headings or horizontal row headings require the [attr]@scope[/] attribute.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-058", "Illegal [attr]@z3998:stage-direction[/] semantic. [hint]Hint: [attr]@z3998:stage-direction[/] is only allowed on [xhtml]<i>[/], [xhtml]<abbr>[/], and [xhtml]<p>[/] elements.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# Check that internal links don't begin with `../`.
	nodes = dom.xpath("/html/body//a[re:test(@href, '^\\.\\./text/')]")
	if nodes:
		messages.append(LintMessage("s-059", "Internal link beginning with [val]../text/[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-060", "Italics on name that requires quotes instead.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-061", "Title and following header content not in a [xhtml]<header>[/] element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-062", "[xhtml]<dt>[/] element in a glossary without exactly one [xhtml]<dfn>[/] child.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-063", "[val]z3998:persona[/] semantic on element that isn’t a [xhtml]<b>[/] or [xhtml]<td>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-065", "[val]fulltitle[/] semantic on element that isn’t in the half title.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-066", "Header element missing [val]se:label[/] semantic.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-067", "Header element with a [val]se:label[/] semantic child, but without an [val]z3998:ordinal[/] semantic child.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-068", "Header element missing [val]z3998:ordinal[/] semantic.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-104", "Header element with incompatible semantics. [hint]Hint: Headers should be either [val]title[/] or [val]z3998:ordinal[/], not both.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-069", "[xhtml]<body>[/] element missing direct child [xhtml]<section>[/] or [xhtml]<article>[/] element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-070", "[xhtml]<h#>[/] element without semantic inflection.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-071", "Sectioning element with more than one heading element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-072", "Element with single [xhtml]<span>[/] child. [hint]Hint: Remove the [xhtml]<span>[/] and promote its attributes to its parent element.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-073", "Header element that requires [val]se:label[/] and [val]z3998:ordinal[/] semantic children.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-074", "[xhtml]<th>[/] element with no text content should be a [xhtml]<td>[/] element instead.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for incorrect children of `<body>`.
	nodes = dom.xpath("/html/body/*[name() != 'section' and name() != 'article' and name() != 'nav']")
	if nodes:
		messages.append(LintMessage("s-075", "[xhtml]<body>[/] element with direct child that isn’t [xhtml]<section>[/], [xhtml]<article>[/], or [xhtml]<nav>[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-076", "Dedication with letter semantics. [hint]Hint: Dedications are never letters, even if they appear to be addressed to someone.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-077", "[xhtml]<header>[/] element preceded by non-sectioning element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-078", "[xhtml]<footer>[/] element followed by non-sectioning element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for elements with no children and only white space contents.
	nodes = dom.xpath("/html/body//*[not(./*) and re:test(., '^\\s+$')]")
	if nodes:
		messages.append(LintMessage("s-079", "Element containing only white space.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-080", "[xhtml]<td>[/] in drama containing both inline text and a block-level element. [hint]Hint: All children of [xhtml]<td>[/] should either be only text, or only block-level elements.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-081", "[xhtml]<p>[/] preceded by [xhtml]<figure>[/], [xhtml]<blockquote>[/xhtml], or [xhtml]<table>[/], but without [val]continued[/] class.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-084", "Poem has incorrect semantics.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-086", "[text]Op. Cit.[/] or [text]Loc. Cit.[/] in endnote. [hint]Hint: [text]Op. Cit.[/] and [text]Loc. Cit.[/] mean [text]the previous reference[/], which usually doesn’t make sense in a popup endnote. Such references should be expanded.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if ebook_flags["has_subtitle"]:
		...
		if nodes:
			...
			messages.append(LintMessage("s-087", "Subtitle in metadata, but no subtitle in the half title page.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))

	else:
		# Make sure we exclude `<a>` because that appears in the ToC landmarks.
		nodes = dom.xpath("/html/body//*[name()!='a' and contains(@epub:type, 'halftitlepage')]//*[contains(@epub:type, 'subtitle')]")
		if nodes:
			messages.append(LintMessage("s-088", "Subtitle in half title page, but no subtitle in metadata.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for MathML without `alttext`.
	nodes = dom.xpath("/html/body//m:math[not(@alttext)]")
	if nodes:
		messages.append(LintMessage("s-089", "MathML missing [attr]@alttext[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-090", "Invalid language tag.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-091", "[xhtml]<span>[/] not followed by [xhtml]<br/>[/] in poetry.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for nested `<abbr>` elements.
	nodes = dom.xpath("/html/body//abbr[./abbr]")
	if nodes:
		messages.append(LintMessage("s-093", "Nested [xhtml]<abbr>[/] element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-094", "Element has an [attr]@xml:lang[/] attribute that incorrectly contains [val]-latn[/] instead of [val]-Latn[/].", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-095", "[xhtml]<p>[/] child of [xhtml]<hgroup>[/] in poetry/verse doesn’t have [css]text-align: center;[/] CSS.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-096", "Heading element in half title page missing [val]fulltitle[/] semantic.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for missing `href` attributes, sometimes a leftover from PG transcriptions.
	nodes = dom.xpath("/html/body//a[not(@href)]")
	if nodes:
		messages.append(LintMessage("s-097", "[xhtml]<a>[/] element without [attr]@href[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("s-098", "[xhtml]<header>[/] element with only one child.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# Check for `@lang` attributes.
	nodes = dom.xpath("//*[@lang]")
	if nodes:
		messages.append(LintMessage("s-102", "[attr]@lang[/] attribute detected. [hint]Hint: Use [attr]@xml:lang[/] instead.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if matches:
		messages.append(LintMessage("s-103", "Probable missing semantics for a roman I numeral.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(matches)))
	...
	if nodes:
		messages.append(LintMessage("s-108", "[xhtml]<thead>[/] element with non-empty [xhtml]<td>[/] descendant. [hint]Hint: Use [xhtml]<th>[/] instead.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
...
def _lint_xhtml_typography_checks(self: 'SachEpub', source_file: SourceFile, dom: EasyXmlTree, special_file: str | None, ebook_flags: dict[str, bool], language: str, missing_files: list[str]) -> tuple[list[LintMessage], list[str]]:
	...
	# Check for punctuation outside quotes. We don't check single quotes because contractions are too common.
	line_matches = source_file.findall(fr"[\p{{Letter}}]+”[,\.](?!{sach.WORD_JOINER} {sach.WORD_JOINER}…)")
	if line_matches:
		messages.append(LintMessage("t-002", "Comma or period outside of double quote. Generally punctuation goes within single and double quotes.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(line_matches)))
	...
	if submessages:
		messages.append(LintMessage("t-003", "[text]“[/] missing matching [text]”[/]. [hint]Hint: When dialog from the same speaker spans multiple [xhtml]<p>[/] elements, it’s correct grammar to omit closing [text]”[/] until the last [xhtml]<p>[/] of dialog.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, submessages))
	...
	if nodes:
		messages.append(LintMessage("t-004", "[text]‘[/] missing matching [text]’[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if filename.name not in IGNORED_FILENAMES:
		...
		if nodes:
			messages.append(LintMessage("t-007", "Possessive [text]’s[/] within name italics. [hint]Hint: If the name in italics is doing the possessing, [text]’s[/] goes outside italics.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	matches += source_file.sub(fr"<td[^>]*?>[”\s{sach.NO_BREAK_SPACE}]+?(<a .+?epub:type=\"noteref\">.+?</a>)?</td>", "").findall(r"(?:“\s*“|”\s*”|’ ’|‘\s*‘).{0,20}")
	matches += source_file.findall(r"[\p{Letter}][,\.:;]\s[,\.:;]\s?[\p{Letter}<].{0,20}")
	matches += source_file.findall(r"[\p{Letter}]\.((</abbr>|</b>|</a>)|</abbr>(</b>|</a>))?\.</p>")
	if matches:
		messages.append(LintMessage("t-008", "Repeated punctuation.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(matches)))
	...
	nodes = dom.xpath(f"/html/body//*[not(ancestor-or-self::*[re:test(@epub:type, '\\btoc\\b')])]/text()[re:test(., '[0-9][^{sach.NO_BREAK_SPACE}]?$')][(following-sibling::abbr[1])[re:test(., '^[ap]\\.m\\.$')]]", str)
	if nodes:
		messages.append(LintMessage("t-009", "Missing no-break space before time and [text]a.m.[/] or [text]p.m.[/].", sach.MESSAGE_TYPE_WARNING, filename, [LintSubmessage(node[-10:] + "<abbr", node.getparent().sourceline) for node in nodes])) # type: ignore `node.getparent()` is provided by lxml "smart strings".
	...
	if submessages:
		messages.append(LintMessage("t-010", "Time set with [text].[/] instead of [text]:[/].", sach.MESSAGE_TYPE_WARNING, filename, submessages))
	...
	if nodes:
		messages.append(LintMessage("t-011", "Missing punctuation before closing quotes.", sach.MESSAGE_TYPE_WARNING, filename, [LintSubmessage(node.to_string()[-30:], node.sourceline) for node in nodes]))
	...
	if nodes:
		messages.append(LintMessage("t-012", "Illegal white space before noteref.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-013", "Roman numeral followed by a period. [hint]Hint: When in mid-sentence Roman numerals must not be followed by a period.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, [LintSubmessage(node.to_string() + ".", node.sourceline) for node in nodes]))

	# Check for two em dashes in a row.
	nodes = dom.xpath(f"/html/body//*[re:test(text(), '—{sach.WORD_JOINER}*—+')]")
	if nodes:
		messages.append(LintMessage("t-014", "Two or more em dashes in a row found. [hint]Hint: Elided words should use the two- or three-em-dash Unicode character, and dialog ending in em dashes should only end in a single em dash.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for money not separated by commas.
	nodes = dom.xpath("/html/body//*[re:test(text(), '[£\\$][0-9]{4,}')]")
	if nodes:
		messages.append(LintMessage("t-015", "Numbers not grouped by commas. [hint]Hint: Separate numbers greater than 1,000 with commas at every three numerals.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-016", "Initials in [xhtml]<abbr epub:type=\"z3998:*-name\">[/] not separated by spaces.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if special_file != "colophon":
		...
		if submessages:
			messages.append(LintMessage("t-017", "Ending punctuation inside formatting like bold, small caps, or italics. [hint]Hint: Ending punctuation is only allowed within formatting if the phrase is an independent clause.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, submessages))
	...
	if nodes:
		messages.append(LintMessage("t-018", "Stage direction ending in period next to other punctuation. [hint]Hint: Remove trailing periods in stage direction.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	line_matches = [match for match in line_matches if "epub:type=\"se:name." not in match[0] and "epub:type=\"z3998:taxonomy" not in match[0] and not regex.match(r"^[\p{Lowercase_Letter}’]+\s“", match[0]) and not regex.match(r"^[\p{Lowercase_Letter}’]+,\s“[\p{Lowercase_Letter}]", sach.formatting.remove_tags(match[0])) and not regex.match(r"^.*?<.+?>[^Ia]<.+?>", match[0])]
	if line_matches:
		messages.append(LintMessage("t-019", "Punctuation outside italics. [hint]Hint: When a complete clause is italicized, ending punctuation except commas must be within containing italics.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(line_matches)))

	# Check for punctuation after endnotes.
	nodes = dom.xpath(f"/html/body//a[contains(@epub:type, 'noteref')][(following-sibling::node()[1])[re:test(., '^[^\\s<–\\]\\)—{sach.WORD_JOINER}a-zA-Z0-9]')]]")
	if nodes:
		messages.append(LintMessage("t-020", "Endnote link inside punctuation. [hint]Hint: Endnote links must be outside of punctuation, including quotation marks.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_node_tags(nodes)))
	...
	matches = source_file.sub(r"(href|id)=\"[^\"]*?\"", "").findall(fr"\b[1-9][0-9]*[{sach.NO_BREAK_SPACE}\-]?(?:[mck]?[mgl]|ft|in|min?|h|sec|hr)\.?\b")
	...
	if matches:
		messages.append(LintMessage("t-021", "Measurement not to standard. [hint]Hint: Numbers are followed by a no-break space and abbreviated units require an [xhtml]<abbr>[/] element. See [path][link=https://standardebooks.org/manual/1.0.0/8-typography#8.8.5]semos://1.0.0/8.8.5[/][/].[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(matches)))

	# Check for `nbsp` within `<abbr epub:type="z3998:*-name">`, which is redundant.
	nodes = dom.xpath(f"/html/body//abbr[re:test(@epub:type, '\\bz3998:[^\\s\"]+name\\b') and contains(text(), '{sach.NO_BREAK_SPACE}')]")
	if nodes:
		messages.append(LintMessage("t-022", "No-break space in [xhtml]<abbr epub:type=\"z3998:*-name\">[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-023", "Comma inside [xhtml]<i>[/] element before closing dialog.", sach.MESSAGE_TYPE_WARNING, filename, [LintSubmessage(node.to_string() + "”", node.sourceline) for node in nodes]))

	# Check for quotation marks in italicized dialog.
	nodes = dom.xpath("/html/body//i[@xml:lang][starts-with(., '“') or re:test(., '”$')]")
	if nodes:
		messages.append(LintMessage("t-024", "Italics outside quotation marks. [hint]Hint: When italicizing language in dialog, italics go inside quotation marks.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	for node in nodes:
		...
		if alt:
			...
			if img_src and img_src.endswith("svg"):
				...
				try:
					svg_path = sach.abspath_relative_to(Path(img_src), filename)
					svg_dom = self.get_dom(svg_path)
					try:
						title_text = svg_dom.xpath("/svg/title")[0].text
					except Exception:
						messages.append(LintMessage("s-027", "[xhtml]<title>[/] element missing.", sach.MESSAGE_TYPE_ERROR, svg_path))
	...
	if svg_mismatched_alts:
		messages.append(LintMessage("s-022", "SVG [xhtml]<title>[/] element doesn’t match its [xhtml]<img>[/] [attr]@alt[/] attribute text.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(svg_mismatched_alts)))


	if img_alt_not_typogrified:
		messages.append(LintMessage("t-025", "Non-typogrified [text]'[/], [text]\"[/] (as [xhtml]&quot;[/]), or [text]--[/] in image [attr]@alt[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(img_alt_not_typogrified)))
	if img_alt_lacking_punctuation:
		messages.append(LintMessage("t-026", "[attr]@alt[/] attribute doesn’t end with punctuation. [hint]Hint: [attr]@alt[/] attributes must be composed of complete sentences ending in appropriate punctuation.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(img_alt_lacking_punctuation)))
	if img_no_alt:
		messages.append(LintMessage("s-004", "[xhtml]<img>[/] element missing [attr]@alt[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(img_no_alt)))
	...
	if line_matches:
		messages.append(LintMessage("t-028", "Possible mis-curled quotation mark.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(line_matches)))
	...
	if matches:
		messages.append(LintMessage("t-029", "Period followed by lowercase letter. [hint]Hint: Abbreviations require an [xhtml]<abbr>[/] element.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(matches)))
	...
	if nodes:
		messages.append(LintMessage("t-030", "Initialism with spaces or without periods.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	nodes = dom.xpath("/html/body//*[re:test(text(), '\\bA\\s*B\\s*C\\s*\\b')]")
	if nodes:
		messages.append(LintMessage("t-031", "[text]A B C[/] must be set as [text]A.B.C.[/] [hint]Hint: [text]A.B.C.[/] isn’t an abbreviation unless used in the sense of [text]A.B.C. store[/].[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-032", "Initialism or name followed by period. [hint]Hint: Periods go within [xhtml]<abbr>[/]. [xhtml]<abbr>[/]s containing periods that end a clause require the [class]eoc[/] class.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, [LintSubmessage(f"{node.to_string()}.", node.sourceline) for node in nodes]))
	...
	if nodes:
		messages.append(LintMessage("t-033", "Space after dash.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for `<cite>` preceded by em dash.
	nodes = dom.xpath("/html/body//cite[(preceding-sibling::node()[1])[re:test(., '—$')]]")
	if nodes:
		messages.append(LintMessage("t-034", "[xhtml]<cite>[/] element preceded by em dash. [hint]Hint: em dashes go within [xhtml]<cite>[/] elements.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-035", "[xhtml]<cite>[/] element not preceded by space.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# Check for partially obscured years in which the last year is an em dash.
	nodes = dom.xpath("/html/body//p[re:test(.,	 '1\\d{2}⁠—[^“’A-Za-z<]')]")
	if nodes:
		messages.append(LintMessage("t-036", "Em dash used to obscure single digit in year. [hint]Hint: Use a hyphen instead.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-037", "[text]”[/] preceded by space.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# Check obviously miscurled quotation marks.
	nodes = dom.xpath("/html/body//p[re:test(., '“$')]")
	if nodes:
		messages.append(LintMessage("t-038", "[text]“[/] before closing [xhtml]</p>[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-039", "Initialism followed by [text]’s[/]. [hint]Hint: Plurals of initialisms aren’t followed by [text]’[/].[/hint]", sach.MESSAGE_TYPE_WARNING, filename, [LintSubmessage(node.to_string() + "’s", node.sourceline) for node in nodes]))
	...
	if nodes:
		messages.append(LintMessage("t-040", "Subtitle with illegal ending period.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# If we don't include preceding characters, the regex is 6x faster.
	matches = source_file.findall(r"[^…]\s[!?;:,].{0,10}")
	if matches:
		messages.append(LintMessage("t-041", "Illegal space before punctuation.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_matches(matches)))
	...
	if nodes:
		messages.append(LintMessage("t-042", "[text]‘[/] used for Greek dasia or [text]’[/] used for Greek psili. [hint]Hint: Use a single precomposed Unicode character, or a combining mark if a precomposed character doesn’t exist.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-043", "Non-English loan word set in italics, when modern typography omits italics. [hint]Hint: A word may be correctly italicized when emphasis is desired, if the word is meant to be pronounced with an accent, or if the word is part of non-English speech.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# Check for comma after leading `Or` in subtitles.
	nodes = dom.xpath("/html/body//*[contains(@epub:type, 'subtitle') and re:test(text(), '^Or\\s')]")
	if nodes:
		messages.append(LintMessage("t-044", "Leading [text]Or[/] in subtitle missing trailing comma.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-045", "Element has [val]z3998:persona[/] semantic and is also set in italics.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	matches = source_file.findall("῾")
	if matches:
		messages.append(LintMessage("t-046", "[text]῾[/] (U+1FFE) detected. [hint]Hint: Use [text]ʽ[/] (U+02BD) instead.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_matches(matches)))
	...
	if nodes:
		messages.append(LintMessage("t-048", "Chapter opening text in all caps.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))

	# Check for two-em-dashes used for elision instead of three-em-dashes.
	matches = source_file.findall(fr"[^{sach.WORD_JOINER}\p{{Letter}}”]⸺[^“{sach.WORD_JOINER}\p{{Letter}}].*")
	if matches:
		messages.append(LintMessage("t-049", "Two-em-dash used for eliding an entire word. [hint]Hint: Use a three-em-dash.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(matches)))
	...
	if nodes:
		messages.append(LintMessage("t-050", "Possessive [text]’s[/] or [text]’[/] outside of element with [val]z3998:persona[/] semantic.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-051", "Dialog in [xhtml]<p>[/] that continues to the next [xhtml]<p>[/], but the next [xhtml]<p>[/] doesn’t begin with [text]“[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	nodes = dom.xpath(f"/html/body//i[@epub:type='z3998:stage-direction' and re:test(., '[a-z]$') and re:test(., '^[A-Z]') and (not(./following-sibling::node()[normalize-space(.)]) or ./following-sibling::node()[1][re:test(normalize-space(.), '^[^{sach.WORD_JOINER}—;a-z]')])]")
	if nodes:
		messages.append(LintMessage("t-052", "Stage direction without ending punctuation. [hint]Hint: Ending punctuation is optional in stage direction that is an interjection in a parent clause, and such interjections should begin with a lowercase letter.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-053", "Stage direction starting in lowercase letter.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-054", "Epigraphs is entirely non-English, but is set in Roman. [hint]Hint: Set it in italics.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# Check for lone acute accents, which should always be combined or swapped for a more accurate Unicode chararacter.
	matches = source_file.findall("´")
	if matches:
		messages.append(LintMessage("t-055", "Lone acute accent ([val]´[/]). [hint]Hint: Use a more accurate Unicode character like prime for coordinates or measurements, or combining accent or breathing mark for Greek text.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_matches(matches)))

	# Check for degree confusable.
	nodes = dom.xpath("/html/body//*[re:test(text(), '[0-9]+º')]")
	if nodes:
		messages.append(LintMessage("t-056", "Masculine ordinal indicator ([val]º[/]) used instead of degree symbol ([val]°[/]). [hint]Hint: The masculine ordinal indicator may be appropriate for ordinal numbers read as Latin, i.e. [val]12º[/] reading [val]duodecimo[/].[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-057", "[xhtml]<p>[/] starting with lowercase letter. [hint]Hint: [xhtml]<p>[/] that continues text after a [xhtml]<blockquote>[/] requires the [class]continued[/] class; and use [xhtml]<br/>[/] to split one clause over many lines.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# Check for illegal characters.
	matches = source_file.findall(fr"({sach.UNICODE_BOM}|{sach.SHY_HYPHEN})")
	if matches:
		messages.append(LintMessage("t-058", "Illegal character.", sach.MESSAGE_TYPE_ERROR, filename, [LintSubmessage(m[0].encode("unicode_escape").decode().replace("\\u", "U+").upper(), m[1]) for m in matches]))
	...
	if nodes:
		messages.append(LintMessage("t-059", "Period at the end of [xhtml]<cite>[/] element before endnote backlink.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-060", "Old style Bible citation.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-061", "Summary-style bridgehead without ending punctuation.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if not dom.xpath("//nav[contains(@epub:type, 'toc')]"):
		...
		if nodes:
			messages.append(LintMessage("t-062", "Uppercased [text]a.m.[/] and [text]p.m.[/]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if incorrectly_cased_titles:
		messages.append(LintMessage("t-064", "Title not correctly titlecased. [hint]Hint: Non-English titles should have an [attr]@xml:lang[/] attribute as they have different titlecasing rules.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, incorrectly_cased_titles))
	...
	if nodes:
		messages.append(LintMessage("t-065", "Header ending in a period.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-066", "Regnal ordinal preceded by [text]the[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-067", "Plural [val]z3998:grapheme[/], [val]z3998:phoneme[/], or [val]z3998:morpheme[/] formed without apostrophe ([text]’[/]).", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-068", "Citation not offset with em dash.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-069", "[xhtml]<cite>[/] in epigraph starting with an em dash.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-070", "[xhtml]<cite>[/] in epigraph ending in a period.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	for node in dom.xpath("/html/body//*[@xml:lang='grc' or @xml:lang='el']"):
		...
		expected_text = sach.typography.normalize_greek(node_text)
	...
	if greek_transcription_errors:
		messages.append(LintMessage("t-073", "Possible transcription error in Greek. [hint]Hint: Use [command]sach[/] [subcommand]unicode-names[/] to see differences in Unicode characters.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, greek_transcription_errors))
	...
	if nodes:
		messages.append(LintMessage("t-074", "Extended sound using hyphen-minus [text]-[/] instead of non-breaking hyphen [text]‑[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		...
		dictionary = sach.spelling.initialize_dictionary()
	...
	if filtered_nodes:
		messages.append(LintMessage("t-075", "Word in verse with acute accent for scansion instead of grave accent.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(filtered_nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-076", "Grapheme or phoneme not italicized. [hint]Hint: Dialect with missing letters should mark missing letters with [text]’[/].[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	nodes = dom.xpath("/html/body//text()[re:test(., '[:;!\\?\\.][“‘]')]", str)
	if nodes:
		messages.append(LintMessage("t-077", "Punctuation followed by opening quotation.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("t-005", "Newspaper name not italicized.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
...
def _lint_xhtml_xhtml_checks(source_file: SourceFile, dom: EasyXmlTree) -> list[LintMessage]:
	...
	if nodes:
		messages.append(LintMessage("x-002", "Uppercase in attribute value. [hint]Hint: Attribute values must be all lowercase.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	nodes = dom.xpath("//*[re:test(@id, '^[0-9]+')]")
	if nodes:
		messages.append(LintMessage("x-007", "Illegal [attr]@id[/] attribute starting with a number.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for double greater-than at the end of a closing tag.
	matches = source_file.findall(r"(>>|>&gt;)")
	if matches:
		messages.append(LintMessage("x-008", "Illegal stray ending [text]>[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(matches)))
	...
	if nodes:
		messages.append(LintMessage("x-009", "Illegal leading 0 in [attr]@id[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for elements in `<title>` tags.
	nodes = dom.xpath("/html/head/title/*")
	if nodes:
		messages.append(LintMessage("x-010", "Illegal element in [xhtml]<title>[/] element.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	if nodes:
		messages.append(LintMessage("x-011", "Illegal underscore in attribute. [hint]Hint: Use dashes instead of underscores.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for style attributes.
	nodes = dom.xpath("/html/body//*[@style]")
	if nodes:
		messages.append(LintMessage("x-012", "Illegal [attr]@style[/] attribute. [hint]Hint: Don’t use inline styles; any element can be targeted with a thoughtful selector.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for illegal elements in `<head>`.
	nodes = dom.xpath("/html/head/*[not(self::title) and not(self::link[@rel='stylesheet'])]")
	if nodes:
		messages.append(LintMessage("x-015", "Illegal element in [xhtml]<head>[/]. [hint]Hint: Only [xhtml]<title>[/] and [xhtml]<link rel=\"stylesheet\">[/] are allowed.[/hint]", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))

	# Check for `xml:lang` attribute starting in uppercase.
	nodes = dom.xpath("//*[re:test(@xml:lang, '^[A-Z]')]")
	if nodes:
		messages.append(LintMessage("x-016", "[attr]@xml:lang[/] attribute with value starting in uppercase letter.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_node_tags(nodes)))
	...
	unexpected_ids = sach.formatting.find_unexpected_ids(dom)
	if unexpected_ids:
		messages.append(LintMessage("x-019", "Unexpected value of [attr]@id[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, [LintSubmessage(f"Found: {unexpected_id[0].get_attr('id')}\nExpected: {unexpected_id[1]}", unexpected_id[0].sourceline) for unexpected_id in unexpected_ids]))
	...
	if nodes:
		messages.append(LintMessage("x-021", "[xhtml]<figure>[/] element with no [attr]@id[/] attribute.", sach.MESSAGE_TYPE_ERROR, filename, LintSubmessage.from_nodes(nodes)))
...
def _lint_xhtml_typo_checks(source_file: SourceFile, dom: EasyXmlTree, special_file: str | None) -> list[LintMessage]:
	...
	if special_file != "titlepage":
		...
		if typo_matches:
			messages.append(LintMessage("y-001", "Possible typo: Doubled [text]a/the/and/of/or/as/if[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(typo_matches)))
	...
	if typos:
		messages.append(LintMessage("y-002", "Possible typo: Punctuation followed directly by a letter, without a space.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-003", "Possible typo: Paragraph missing ending punctuation.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	typos = dom.xpath("//p[re:test(., '-[‘“]')]")
	if typos:
		messages.append(LintMessage("y-004", "Possible typo: Mis-curled quotation mark after dash.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if special_file != "colophon":
		...
		if typos:
			messages.append(LintMessage("y-005", "Possible typo: Punctuation followed by period or comma.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-006", "Possible typo: [text]‘[/] without matching [text]’[/]. [hint]Hint: [text]’[/] are used for elisions.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typo_messages:
		messages.append(LintMessage("y-007", "Possible typo: [text]‘[/] not within [text]“[/]. [hint]Hint: Should [text]‘[/] be replaced with [text]“[/]? Is there a missing closing quote? Is this a nested quote that should be preceded by [text]“[/]? Are quotes in close proximity correctly closed?[/hint]", sach.MESSAGE_TYPE_WARNING, filename, typo_messages))

	# Check for single quotes when there should be double quotes in an interjection in dialog.
	typos = dom.xpath("//p[re:test(., '“[^”]+?’⁠—[^”]+?—“')]")
	if typos:
		messages.append(LintMessage("y-008", "Possible typo: Dialog interrupted by interjection but with incorrect closing quote.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-009", "Possible typo: Dialog begins with lowercase letter.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-010", "Possible typo: Comma ending dialogue.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	typos = dom.xpath("/html/body//p[re:test(., '’{2,}')]")
	if typos:
		messages.append(LintMessage("y-011", "Possible typo: Two or more [text]’[/] in a row.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	typos = dom.xpath("/html/body//p[re:test(., '”[A-Za-z]')]")
	if typos:
		messages.append(LintMessage("y-012", "Possible typo: [text]”[/] directly followed by letter.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-013", "Possible typo: Punctuation not within [text]’[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-014", "Possible typo: Unexpected [text].[/] at the end of quotation. [hint]Hint: If a dialog tag follows, should this be [text],[/]?[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	# Check for some common OCR misspellings.
	typos = source_file.findall(r"\bbad (?:been|seen)\b|\scf\s")
	if typos:
		messages.append(LintMessage("y-015", "Possible typo: Misspelled word.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(typos)))

	# Check for two periods in a row, almost always a typo for one period or a `hellip`.
	typos = dom.xpath("/html/body//p[re:test(., '[^\\.]\\.\\.[^\\.]')]")
	if typos:
		messages.append(LintMessage("y-016", "Possible typo: Consecutive periods ([text]..[/]).", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	# Check for `ldquo` followed by space.
	typos = dom.xpath("/html/body//p[re:test(., '“\\s+[^‘’ʻ]')]")
	if typos:
		messages.append(LintMessage("y-017", "Possible typo: [text]“[/] followed by space.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	# Check for `lsquo` followed by space.
	typos = dom.xpath("/html/body//p[re:test(., '‘\\s+[^“’]')]")
	if typos:
		messages.append(LintMessage("y-018", "Possible typo: [text]‘[/] followed by space.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-019", "Possible typo: [text]”[/] without opening [text]“[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	# Check for `,.`.
	typos = dom.xpath("/html/body//p[re:test(., ',\\.')]")
	if typos:
		messages.append(LintMessage("y-020", "Possible typo: Consecutive comma-period ([text],.[/]).", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	typos = dom.xpath("/html/body//p[re:test(., '“[^‘”]+’$')]")
	if typos:
		messages.append(LintMessage("y-021", "Possible typo: Closing [text]’[/] without opening [text]‘[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	# Check for a paragraph consisting entirely of two quotations.
	typos = dom.xpath("/html/body//p[re:test(., '^“[^”]+?”\\s“[^”]+?”$')]")
	if typos:
		messages.append(LintMessage("y-022", "Possible typo: Consecutive quotations without intervening text, e.g. [text]“…” “…”[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-023", "Possible typo: Question mark within words.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	# Check for dashes instead of em dashes.
	typos = dom.xpath("/html/body//p[re:test(., '\\s[a-z]+-(the|there|is|and|or|they|when)\\s')]")
	if typos:
		messages.append(LintMessage("y-024", "Possible typo: Dash before [text]the/there/is/and/or/they/when[/] probably should be em dash.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-025", "Possible typo: Letter/comma/quote mark/letter with no intervening space.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	typos = dom.xpath(f"/html/body//p[not(parent::hgroup) and re:test(., '\\b[a-z]+\\s(But|And|For|Nor|Yet|Or)\\b[^’\\.\\?\\-{sach.WORD_JOINER}]') and not(./*[(name()='i' or name()='q') and re:test(., '^(But|And|For|Nor|Yet|Or)\\b')])]")
	if typos:
		messages.append(LintMessage("y-026", "Possible typo: No punctuation before conjunction [text]But/And/For/Nor/Yet/Or[/].", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))

	# Check for extra closing single quote at the end of dialog.
	typos = dom.xpath("/html/body//p[re:test(., '^“[^‘]+”\\s*’$')]")
	if typos:
		messages.append(LintMessage("y-027", "Possible typo: Extra [text]’[/] at end of paragraph.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-028", "Possible typo: [xhtml]<abbr>[/] directly preceded or followed by letter.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-029", "Possible typo: Italics followed by a letter.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if typos:
		messages.append(LintMessage("y-030", "Possible typo: Lowercase quotation following a period. [hint]Hint: Check either that the period should be a comma, or that the quotation should start with a capital.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(typos)))
	...
	if nodes:
		messages.append(LintMessage("y-031", "Possible typo: Dialog tag missing punctuation.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("y-032", "Possible typo: Italics running into preceding or following characters.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	# Check for three-em-dashes not preceded by a space.
	nodes = dom.xpath(f"/html/body//p[re:test(., '[^>“(\\s{sach.WORD_JOINER}]{sach.WORD_JOINER}?⸻')]")
	if nodes:
		messages.append(LintMessage("y-033", "Possible typo: Three-em-dash obscuring an entire word, but not preceded by a space.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("y-034", "Possible typo: [text].[/] embedded in word. [hint]Hint: Abbreviations must be in an [xhtml]<abbr>[/] element.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("y-035", "Possible typo: Single letter. [hint]Hint: Does this need [val]z3998:grapheme[/] or [val]z3998:phoneme[/] or [attr]@xml:lang[/] semantics? Is this dialect requiring [text]’[/] to signify an elided letter?[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if nodes:
		messages.append(LintMessage("y-036", "Possible typo: [text]’[/] after punctuation, but no [text]‘[/]. [hint]Hints: Is [text]‘[/] missing or mis-curled? [text]’[/] that elides a word is placed before punctuation.[/hint]", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))
	...
	if line_matches:
		messages.append(LintMessage("y-037", "Possible typo: Dialog without ending comma.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_matches(line_matches)))

	nodes = dom.xpath("/html/body//text()[re:test(., '[^<][!?:;]-')]", str)
	if nodes:
		messages.append(LintMessage("y-038", "Possible typo: Dash after punctuation.", sach.MESSAGE_TYPE_WARNING, filename, LintSubmessage.from_nodes(nodes)))

	return messages

def _lint_image_metadata_checks(self: 'SachEpub', has_images: bool) -> list[LintMessage]:
	"""
	...
	if has_images:
		if not has_visual_accessmode:
			messages.append(LintMessage("m-028", "Images found in ebook, but no [val]schema:accessMode[/] property set to [val]visual[/] in metadata.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))

		if not has_accessibility_feature_alt:
			messages.append(LintMessage("m-029", "Images found in ebook, but no [val]schema:accessibilityFeature[/] property set to [val]alternativeText[/] in metadata.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))

		if not has_wat_role:
			messages.append(LintMessage("m-040", "Images found in ebook, but no [attr]@role[/] property set to [val]wat[/] in metadata for the writer of the alt text.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))

	if not has_images:
		if has_visual_accessmode:
			messages.append(LintMessage("m-038", "[val]schema:accessMode[/] property set to [val]visual[/], but no images in ebook.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(visual_accessmode)))

	return messages

def _lint_process_ignore_file(self: 'SachEpub', lint_ignore_dom: EasyXmlTree | None, allowed_messages: list[str] | None, messages: list[LintMessage]) -> list[LintMessage]:
	"""
	...
	if lint_ignore_dom:
		...
		if nodes:
			messages.append(LintMessage("m-039", "[xml]<file>[/] element with duplicate [attr]@path[/] attribute value.", sach.MESSAGE_TYPE_ERROR, lint_ignore_path, LintSubmessage.from_node_tags(nodes)))
		...
		if nodes:
			messages.append(LintMessage("m-088", "[xhtml]<ignore>[/] element ignores a specific line, but a sibling [xhtml]<ignore>[/] element ignores the entire file.", sach.MESSAGE_TYPE_ERROR, lint_ignore_path, LintSubmessage.from_node_tags(nodes)))

		nodes = lint_ignore_dom.xpath("./file[re:test(@path, '[\\*\\?\\!\\[\\]]') and ./ignore/line]")
		if nodes:
			messages.append(LintMessage("m-046", "[xml]<file>[/] element contains wildcard, but [xml]<ignore>[/] child specifies a line number.", sach.MESSAGE_TYPE_ERROR, lint_ignore_path, LintSubmessage.from_node_tags(nodes)))

		nodes = lint_ignore_dom.xpath("./file/ignore[./line[text() = preceding-sibling::line/text() or text() = following-sibling::line/text()]]")
		if nodes:
			messages.append(LintMessage("m-089", "Ignore rule ignores the same line more than once.", sach.MESSAGE_TYPE_ERROR, lint_ignore_path, LintSubmessage.from_node_tags(nodes)))
		...
		if ignores_with_illegal_paths:
			messages.append(LintMessage("m-047", "Illegal path. [hint]Hint: Ignoring [path]*[/] is too general; target specific files.[/hint]", sach.MESSAGE_TYPE_WARNING, lint_ignore_path, LintSubmessage.from_node_tags(ignores_with_illegal_paths)))
	...
	if ignored_codes:
		...
		if unused_codes:
			messages.append(LintMessage("m-048", "Unused ignore rule.", sach.MESSAGE_TYPE_ERROR, lint_ignore_path, LintSubmessage.from_node_text(unused_codes)))

	return messages


def lint(self: 'SachEpub', skip_lint_ignore: bool, allowed_messages: list[str] | None = None) -> list[LintMessage]:
	"""
	...
	if not skip_lint_ignore and lint_ignore_path.exists():
		with importlib.resources.as_file(importlib.resources.files("sach.data").joinpath("se-lint-ignore.rng")) as rng_path:
			...
			try:
				...
				raise sach.InvalidFileException(f"[path][link=file://{lint_ignore_path.resolve()}]se-lint-ignore.xml[/][/] is invalid: {ex}")

	# Cache the browser default stylesheet for later use.
	with importlib.resources.files("sach.data").joinpath("browser.css").open("r", encoding="utf-8") as css:
	...
	if missing_spine_files:
		for missing_spine_file in missing_spine_files:
			messages.append(LintMessage("f-007", "File not listed in [xml]<spine>[/].", sach.MESSAGE_TYPE_ERROR, missing_spine_file))
	...
	try:
		...
		raise sach.InvalidSachEbookException(f"Missing [xml]<dc:language>[/] element in [path][link=file://{self.metadata_file_path}]{self.metadata_file_path.name}[/][/].") from ex
	...
	try:
		...
		raise sach.InvalidSachEbookException(f"Couldn’t open [path]{local_css_path}[/].") from ex
	...
	if self.is_se_ebook:
		...
		for illegal_file in illegal_files:
			messages.append(LintMessage("f-001", "Illegal file or directory.", sach.MESSAGE_TYPE_ERROR, Path(illegal_file)))

	# Check for repeated punctuation.
	nodes = self.metadata_dom.xpath("/package/metadata/*[re:test(., '[,;]{2,}')]")
	if nodes:
		messages.append(LintMessage("t-008", "Repeated punctuation.", sach.MESSAGE_TYPE_WARNING, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	nodes = self.metadata_dom.xpath(f"/package/metadata//text()[re:test(., '[{sach.NO_BREAK_SPACE}{sach.HAIR_SPACE} ]{{2,}}')]", str)
	if nodes:
		messages.append(LintMessage("t-001", "Illegal double spacing. [hint]Hint: Sentences should be single-spaced. Spaces might include Unicode hair spaces and no-break spaces.[/hint]", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, LintSubmessage.from_nodes(nodes)))
	...
	if self.is_se_ebook:
		try:
			with importlib.resources.as_file(importlib.resources.files("sach.data.templates").joinpath("LICENSE.md")) as license_file_path:
		...
		try:
			with importlib.resources.as_file(importlib.resources.files("sach.data.templates").joinpath("core.css")) as core_css_file_path:
		...
		try:
			with importlib.resources.as_file(importlib.resources.files("sach.data.templates").joinpath("logo.svg")) as logo_svg_file_path:
		...
		try:
			with importlib.resources.as_file(importlib.resources.files("sach.data.templates").joinpath("uncopyright.xhtml")) as uncopyright_file_path:
		...
		try:
			with importlib.resources.as_file(importlib.resources.files("sach.data.templates").joinpath("sach.css")) as se_css_file_path:
				if not _file_matches_template(se_css_file_path, self.content_path / "css/sach.css"):
					files_not_matching_templates.append(self.content_path / "css/sach.css")

		except Exception:
			missing_files.append("css/sach.css")
	...
	for root, directories, filenames in os.walk(self.path):
		...
		for directory in natsorted(directories):
			...
			url_safe_filename = sach.formatting.make_url_safe(directory)
			if directory != url_safe_filename:
				directories_not_url_safe.append(Path(root) / directory)

		for filename in natsorted(filenames):
			file_path = sach.abspath_relative_to(Path(filename), Path(root))

			if file_path.stem != "LICENSE":
				if file_path.stem == "cover.source":
					ebook_flags["has_cover_source"] = True
				else:
					url_safe_filename = sach.formatting.make_url_safe(file_path.stem) + file_path.suffix
					if file_path.name != url_safe_filename:
						files_not_url_safe.append(file_path)

			if "-0" in file_path.name:
				messages.append(LintMessage("f-009", "Illegal leading [text]0[/] in filename.", sach.MESSAGE_TYPE_ERROR, file_path))
			...
			try:
				...
				messages.append(LintMessage("f-010", "Problem decoding file as utf-8.", sach.MESSAGE_TYPE_ERROR, file_path))
				continue
			...
			if file_path.suffix == ".xml" and source_file.findall("http://www.musicxml.org/dtds/"):
				messages.append(LintMessage("f-004", "MusicXML files must end in [path].musicxml[/].", sach.MESSAGE_TYPE_ERROR, file_path))

			if file_path.suffix == ".musicxml" and not self.metadata_dom.xpath("/package/metadata/meta[(@property='role') and text()='mcp']"):
				messages.append(LintMessage("m-092", "MusicXML files found, but no MARC relator [val]mcp[/] (Music copyist).", sach.MESSAGE_TYPE_WARNING, file_path))
			...
			if not file_path.is_relative_to(self.path / "images"):
				...
				if line_matches:
					messages.append(LintMessage("x-001", "[text]utf-8[/] string incorrectly cased. [hint]Hint: [text]utf-8[/] must always be lowercase.[/hint]", sach.MESSAGE_TYPE_ERROR, file_path, LintSubmessage.from_matches(line_matches)))
			...
			if file_path.suffix == ".xml":
				...
				if xml_dom.xpath("/search-key-map") and file_path.name != "glossary-search-key-map.xml":
					messages.append(LintMessage("f-013", "Glossary search key map must be named [path]glossary-search-key-map.xml[/].", sach.MESSAGE_TYPE_ERROR, file_path))
			...
			if file_path.suffix == ".xhtml":
				...
				if nodes and file_path.stem not in [node.get_attr("id") for node in nodes]:
					messages.append(LintMessage("f-015", "Filename doesn’t match [attr]@id[/] attribute of primary [xhtml]<section>[/] or [xhtml]<article>[/]. [hint]Hint: [attr]@id[/] attributes don’t include the file extension.[/hint]", sach.MESSAGE_TYPE_ERROR, file_path, LintSubmessage.from_node_tags([nodes[0]])))
				...
				if dom.xpath("/html/head/link[contains(@href, 'local.css')]"):
					for selector in local_css_selectors:
						...
						except sach.NotImplementedException:
							# This gets thrown on some selectors not yet implemented by lxml, like `*:first-of-type`.
							unused_selectors.remove(selector)
							continue
						except Exception as ex:
							raise sach.InvalidCssException(f"Couldn’t parse CSS in or near this line: [css-selector]{selector}[/]: {ex}")
				...
				nodes = dom.xpath(f"//text()[re:test(., '[{sach.NO_BREAK_SPACE}{sach.HAIR_SPACE} ]{{2,}}') and not(ancestor::td[re:test(., '”[{sach.NO_BREAK_SPACE}{sach.HAIR_SPACE} ]+”')])]", str)
				if nodes:
					if len(nodes) <= 10:
						...
						messages.append(LintMessage("t-001", "Illegal double spacing. [hint]Hint: Sentences should be single-spaced. Spaces might include Unicode hair spaces and no-break spaces.[/hint]", sach.MESSAGE_TYPE_ERROR, file_path, LintSubmessage.from_nodes(nodes)))
					else:
						# Only list the filename.
						messages.append(LintMessage("t-001", "Illegal double spacing. [hint]Hint: Sentences should be single-spaced. Spaces might include Unicode hair spaces and no-break spaces.[/hint]", sach.MESSAGE_TYPE_ERROR, file_path))
	...
	if self.cover_path and cover_svg_title != titlepage_svg_title:
		messages.append(LintMessage("s-028", "[xhtml]<title>[/] elements in cover SVG and titlepage SVG don’t match.", sach.MESSAGE_TYPE_ERROR, self.cover_path))

	if ebook_flags["has_frontmatter"] and not ebook_flags["has_halftitle"]:
		messages.append(LintMessage("s-020", "Frontmatter found, but no half title page.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))
	...
	if ebook_flags["has_mathml"]:
		...
		if not self.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessibilityFeature' and text() = 'describedMath']"):
			messages.append(LintMessage("m-077", "MathML found in ebook, but no [val]schema:accessibilityFeature[/] properties set to [val]MathML[/] and [val]describedMath[/] in metadata.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))
	...
	for css_class, nodes_tuples in xhtml_css_classes.items():
		...
		if css_class not in IGNORED_CLASSES and f".{css_class}" not in self.local_css:
			...
			messages.append(LintMessage("x-013", "CSS class found in XHTML, but not any CSS file.", sach.MESSAGE_TYPE_ERROR, nodes_tuple[0], LintSubmessage.from_node_tags([nodes_tuple[1]])))
		...
		if len(nodes_tuples) == 1 and css_class not in IGNORED_CLASSES and not regex.match(r"^i[0-9]+$", css_class):
			...
			nodes_tuple = nodes_tuples[0]
			messages.append(LintMessage("c-008", "CSS class only used once. [hint]Hint: Craft a selector instead of a single-use class.[/hint]", sach.MESSAGE_TYPE_WARNING, nodes_tuple[0], LintSubmessage.from_node_tags([nodes_tuple[1]])))
	...
	if unused_id_attrs:
		...
		for _, (file_path, nodes_list) in enumerate(unused_id_attrs):
			if nodes_list:
				messages.append(LintMessage("x-018", "Unused [attr]@id[/] attribute.", sach.MESSAGE_TYPE_ERROR, file_path, LintSubmessage.from_node_tags(nodes_list)))
	...
	if files_not_url_safe:
		...
		for filepath_string in files_not_url_safe_strings:
			...
			url_safe_filename = sach.formatting.make_url_safe(filepath.stem) + filepath.suffix
			messages.append(LintMessage("f-008", "Filename isn’t URL-safe.", sach.MESSAGE_TYPE_ERROR, filepath, [LintSubmessage(f"Found: {filepath.name}\nExpected: {url_safe_filename}")]))
	...
	if directories_not_url_safe:
		...
		for filepath_string in directories_not_url_safe_strings:
			filepath = Path(filepath_string)
			url_safe_filename = sach.formatting.make_url_safe(filepath.stem)
			messages.append(LintMessage("f-008", f"Filename isn’t URL-safe. Expected: [path]{url_safe_filename}[/].", sach.MESSAGE_TYPE_ERROR, filepath))
	...
	if id_nodes:
		...
		for _, nodes_tuples in duplicate_ids.items():
			...
			messages.append(LintMessage("x-017", "[attr]@id[/] attribute value used more than once in ebook.", sach.MESSAGE_TYPE_ERROR, file_path, LintSubmessage.from_node_tags(nodes)))
	...
	for file_string, headings_list in missing_headings.items():
		messages.append(LintMessage("m-045", "Heading not found in the ToC.", sach.MESSAGE_TYPE_ERROR, Path(file_string), headings_list))
	...
	if missing_styles:
		messages.append(LintMessage("c-006", "Semantic found, but missing corresponding CSS style.", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_node_tags(missing_styles)))

	if missing_files:
		messages.append(LintMessage("f-002", "Missing expected file or directory.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path, missing_files))
	...
	if unused_selectors:
		...
		messages.append(LintMessage("c-002", "Unused CSS selectors.", sach.MESSAGE_TYPE_ERROR, local_css_path, LintSubmessage.from_matches(matches, formatting_tag="css-selector")))

	if short_story_count and not self.metadata_dom.xpath("//meta[@property='schema:genre' and text() = 'Shorts']"):
		messages.append(LintMessage("m-027", "[val]se:short-story[/] semantic inflection found, but no [val]schema:genre[/] with the value of [val]Shorts[/].", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))

	if is_drama and not ebook_flags["has_drama_table"]:
		messages.append(LintMessage("m-095", "[val]schema:genre[/] is [val]Drama[/], but no [val]z3998:drama[/] element.", sach.MESSAGE_TYPE_ERROR, self.metadata_file_path))

	if unused_glossary_entries:
		messages.append(LintMessage("m-070", "Glossary entry not found in the text.", sach.MESSAGE_TYPE_ERROR, self.content_path / "glossary-search-key-map.xml", LintSubmessage.from_nodes(unused_glossary_entries)))
	...
	if does_ebook_look_like_collection:
		...
		if not nodes:
			messages.append(LintMessage("m-079", "Ebook looks like a collection, but no [xml]<meta property=\"schema:additionalType\">http://schema.org/Collection</meta>[/] element in metadata.", sach.MESSAGE_TYPE_WARNING, self.metadata_file_path))

	for file_not_matching_template in files_not_matching_templates:
		messages.append(LintMessage("f-003", "File doesn’t match template.", sach.MESSAGE_TYPE_ERROR, file_not_matching_template))
