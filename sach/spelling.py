...
def get_xhtml_language(xhtml: str) -> str:
	...
	if language not in supported_languages:
		raise sach.InvalidLanguageException(f"No valid [attr]@xml:lang[/] attribute in [xhtml]<html>[/] element. Only [text]{'[/], [text]'.join(supported_languages[:-1])}[/], and [text]{supported_languages[-1]}[/] are supported.")
...
def initialize_dictionary() -> set[str]:
	...
	if not DICTIONARY:
		with importlib.resources.files("sach.data").joinpath("words").open("r", encoding="utf-8") as dictionary:
