...
import sach # pylint: disable=cyclic-import
import sach.css # pylint: disable=cyclic-import

CSS_SELECTOR_CACHE: dict[str, cssselect.CSSSelector] = {}
CSS_RULES_CACHE: dict[str, list[sach.css.CssRule]] = {}
...
def escape_xpath(string: str) -> str:
	...
	```python
	dom.xpath(f"//title[text() = {sach.easy_xml.escape_xpath(title)}]")
	```
	"""
...
class EasyXmlTree:
	...
	def __init__(self, xml: str | bytes | etree.ElementTree):
		...
		try:
			...
			raise sach.InvalidXmlException(f"Couldn’t parse XML: {ex}") from ex
	...
	@staticmethod
	def _apply_css_declaration_to_node(node: EasyXmlElement, declaration: sach.css.CssDeclaration, specificity_number: int):
		if declaration.applies_to == "all" or (declaration.applies_to == "block" and node.tag in sach.css.CSS_BLOCK_ELEMENTS):
	...
	def apply_css(self, css: str, filename: str | None = None):
		...
		if filename and filename in CSS_RULES_CACHE:
			...
			rules = sach.css.parse_rules(css)
			if filename:
