...
import sach.vietnamese as vietnamese
...
def typogrify(xhtml: str, smart_quotes: bool = True) -> str:
	...
	# Vietnamese text should be stored in precomposed (NFC) form. If the source
	# is decomposed (NFD) — common with hand-edited Vietnamese files — normalise
	# it to NFC as the house representation before typographic processing so
	# that combining marks are composed into single code points.
	if vietnamese.has_vietnamese_marks(xhtml):
		xhtml = vietnamese.normalize(xhtml)
	...
	# xhtml = xhtml.replace(sach.NO_BREAK_HYPHEN, "-")

	# Replace horizontal bar with em dash.
	xhtml = xhtml.replace("―", "—")

	# Replace space + en dash with em dash.
	xhtml = regex.sub(r"\s–\s?", f"{sach.WORD_JOINER}—", xhtml)
	...
	xhtml = xhtml.replace(sach.WORD_JOINER, "")
	...
	xhtml = regex.sub(r"([\p{Letter}])-“", r"\1—“", xhtml)
	xhtml = regex.sub(r":-</", fr":{sach.WORD_JOINER}—</", xhtml)
	...
	xhtml = regex.sub(fr"([^\s{sach.WORD_JOINER}{sach.NO_BREAK_SPACE}{sach.HAIR_SPACE}])([—⸻])", fr"\1{sach.WORD_JOINER}\2", xhtml)
	...
	xhtml = regex.sub(fr"{sach.WORD_JOINER}?–{sach.WORD_JOINER}?", fr"{sach.WORD_JOINER}–{sach.WORD_JOINER}", xhtml)
	...
	xhtml = regex.sub(fr"([^\s{sach.WORD_JOINER}{sach.NO_BREAK_SPACE}{sach.HAIR_SPACE}])⸺", fr"\1{sach.WORD_JOINER}⸺", xhtml)
	xhtml = regex.sub(fr"⸺([^\s\p{{P}}{sach.WORD_JOINER}])", fr"⸺{sach.WORD_JOINER}\1", xhtml)

	# Add a space between text and `—th`, which is usually an obscured number. I.e. `The —th battalion`.
	xhtml = regex.sub(fr"([\p{{Lowercase_Letter}}]){sach.WORD_JOINER}—th\b", r"\1 —th", xhtml)

	# Remove word joiners from following opening tags - they're usually never correct.
	xhtml = regex.sub(fr"<([\p{{Letter}}]+)([^>]*?)>{sach.WORD_JOINER}", r"<\1\2>", xhtml)

	# Add a word joiner after em dashes within `<cite>` elements.
	xhtml = regex.sub(r"<cite([^>]*?)>—", fr"<cite\1>—{sach.WORD_JOINER}", xhtml)

	# Finally fix some other mistakes.
	xhtml = xhtml.replace("—-", "—")
	xhtml = regex.sub(r"\s—\s", f"{sach.WORD_JOINER}—{sach.WORD_JOINER}", xhtml)
	...
	xhtml = regex.sub(fr"([I\p{{Lowercase_Letter}}>\.\?\!,’]{sach.WORD_JOINER})⸺”", r"\1—”", xhtml)
	xhtml = regex.sub(fr"([^\s‘“—>][a-z\.]{sach.WORD_JOINER})⸺\s?", r"\1—", xhtml)

	# Some older texts use the `,—` construct; remove that archaism.
	xhtml = regex.sub(fr",({sach.WORD_JOINER}?)—", r"\1—", xhtml)

	# Remove spaces after two-em-dashes that do not appear to be elision.
	xhtml = regex.sub(fr"(\p{{Letter}}{{2,}}{sach.WORD_JOINER})⸺\s", r"\1—", xhtml)

	# Dash before closing double quote in clauses with more than two words is almost always a typo.
	xhtml = regex.sub(r"(“[^<]+?\s[^<]+?)-”", fr"\1{sach.WORD_JOINER}—”", xhtml)

	# Remove `nbsp`s between words.
	xhtml = regex.sub(fr"([^>…]){sach.NO_BREAK_SPACE}([\p{{Letter}}\p{{Digit}}])", r"\1 \2", xhtml)

	# Ensure `Mr.`, `Mrs.`, and other abbreviations have a trailing period, and include a non-breaking space.
	xhtml = regex.sub(r"\b(Mr|Mr?s|Drs?|Profs?|Lieut|Fr|Lt|Capt|Pvt|Esq|Mt|MM|Mmes?|Mlles?|Hon|Mdlle)\.?(</abbr>)?\s+", fr"\1.\2{sach.NO_BREAK_SPACE}", xhtml)
	...
	xhtml = regex.sub(r"""(?<!(?:st|nd|rd|th) )(St\.)\s+([A-Z])""", fr"\1{sach.NO_BREAK_SPACE}\2", xhtml)
	...
	xhtml = regex.sub(r"\bMon\.(</abbr>)?\s+", fr"Mon.\1{sach.NO_BREAK_SPACE}", xhtml)
	...
	xhtml = regex.sub(fr"\b(Mr|Mr?s|Drs?|Profs?|Lieut|Fr|Lt|Capt|Pvt|Esq|Mt|St|MM|Mmes?|Mlles?)\.?(</abbr>)?([^\P{{Punctuation}}\.]|{sach.WORD_JOINER})", r"\1.\2\3", xhtml)
	...
	xhtml = regex.sub(fr"""<abbr epub:type="(z3998:[^"\s]+?name)">St\.{sach.NO_BREAK_SPACE}([A-Z])""", r"""<abbr epub:type="\1">St. \2""", xhtml)

	xhtml = regex.sub(r"\bNo(s?)\.\s+([0-9]+)", fr"No\1.{sach.NO_BREAK_SPACE}\2", xhtml)
	xhtml = regex.sub(r"<abbr>No\.</abbr>\s+", fr"<abbr>No.</abbr>{sach.NO_BREAK_SPACE}", xhtml)

	xhtml = regex.sub(r"([0-9]+)\s<abbr", fr"\1{sach.NO_BREAK_SPACE}<abbr", xhtml)
	...
	# Remove double spaces that use `sach.NO_BREAK_SPACE` for spacing.
	xhtml = regex.sub(fr"{sach.NO_BREAK_SPACE}[{sach.NO_BREAK_SPACE} ]+", r" ", xhtml)
	xhtml = regex.sub(fr" [{sach.NO_BREAK_SPACE} ]+", r" ", xhtml)
	...
	xhtml = regex.sub(r"([^>])De ([A-Z][a-z]+?)", fr"\1De{sach.NO_BREAK_SPACE}\2", xhtml)
	...
	xhtml = regex.sub(fr"“[\s{sach.NO_BREAK_SPACE}]*‘", fr"“{sach.HAIR_SPACE}‘", xhtml)
	xhtml = regex.sub(fr"’[\s{sach.NO_BREAK_SPACE}]*”", fr"’{sach.HAIR_SPACE}”", xhtml)
	xhtml = regex.sub(fr"“[\s{sach.NO_BREAK_SPACE}]*’", fr"“{sach.HAIR_SPACE}’", xhtml)
	xhtml = regex.sub(fr"‘[\s{sach.NO_BREAK_SPACE}]*“", fr"‘{sach.HAIR_SPACE}“", xhtml)
	xhtml = regex.sub(fr"‘[\s{sach.NO_BREAK_SPACE}]*’", fr"‘{sach.HAIR_SPACE}’", xhtml)
	...
	xhtml = regex.sub(fr"”[\s{sach.NO_BREAK_SPACE}]*’([^\p{{Letter}}])", fr"”{sach.HAIR_SPACE}’\1", xhtml)

	# Fix ellipses spacing.
	xhtml = regex.sub(r"\s*\.\s*\.\s*\.\s*", r"…", xhtml)
	xhtml = regex.sub(fr"[\s{sach.NO_BREAK_SPACE}]?…[\s{sach.NO_BREAK_SPACE}]?\.", fr".{sach.HAIR_SPACE}…", xhtml)
	xhtml = regex.sub(fr"[\s{sach.NO_BREAK_SPACE}]?…[\s{sach.NO_BREAK_SPACE}]?", fr"{sach.HAIR_SPACE}… ", xhtml)
	xhtml = regex.sub(fr"<p([^>]*?)>{sach.HAIR_SPACE}…", r"<p\1>…", xhtml)

	# Remove spaces between opening tags and ellipses.
	xhtml = regex.sub(fr"(<[\p{{Letter}}0-9]+[^<]+?>)[\s{sach.NO_BREAK_SPACE}]+?…", r"\1…", xhtml)

	# Remove spaces between closing tags and ellipses.
	xhtml = regex.sub(fr"…[\s{sach.NO_BREAK_SPACE}]?(</[\p{{Letter}}0-9]+>)", r"…\1", xhtml)
	xhtml = regex.sub(fr"…[\s{sach.NO_BREAK_SPACE}]+([\)”’])(?![\p{{Letter}}])", r"…\1", xhtml) # If followed by a letter, the single quote is probably a leading elision.
	xhtml = regex.sub(fr"([\(“‘])[\s{sach.NO_BREAK_SPACE}]+…", r"\1…", xhtml)
	xhtml = regex.sub(fr"…[\s{sach.NO_BREAK_SPACE}]?([\!\?\.\;\,])", fr"…{sach.HAIR_SPACE}\1", xhtml)
	xhtml = regex.sub(fr"([\!\?\.\;”’])[\s{sach.NO_BREAK_SPACE}]?…", fr"\1{sach.HAIR_SPACE}…", xhtml)
	xhtml = regex.sub(fr"\,[\s{sach.NO_BREAK_SPACE}]?…", fr",{sach.HAIR_SPACE}…", xhtml)

	# Add `nbsp` to ellipses that open dialog.
	xhtml = regex.sub(r"([“‘])…\s([\p{Letter}0-9])", fr"\1…{sach.NO_BREAK_SPACE}\2", xhtml)
	...
	xhtml = regex.sub(r"([0-9])\s+(oz\.|lbs\.)", fr"\1{sach.NO_BREAK_SPACE}\2", xhtml, flags=regex.IGNORECASE)

	# Add non-breaking spaces between Arabic numbers and `AM`/`PM`.
	xhtml = regex.sub(r"([0-9])\s+([ap])\.m\.", fr"\1{sach.NO_BREAK_SPACE}\2.m.", xhtml, flags=regex.IGNORECASE)
	xhtml = regex.sub(r"([0-9])\s+<abbr([^>]*?)>([ap])\.m\.", fr"\1{sach.NO_BREAK_SPACE}<abbr\2>\3.m.", xhtml, flags=regex.IGNORECASE)
	...
	xhtml = regex.sub(fr"(\s+){sach.WORD_JOINER}—", r"\1—", xhtml)
	...
	# Add an `&nbsp;` before `&amp;`.
	xhtml = regex.sub(r" &amp;", f"{sach.NO_BREAK_SPACE}&amp;", xhtml)

	# Add word joiners to ellipses.
	xhtml = regex.sub(fr"{sach.HAIR_SPACE}…", f"{sach.WORD_JOINER}{sach.HAIR_SPACE}{sach.WORD_JOINER}…", xhtml)

	# Remove any extra spaces in 4-dot ellipses that occur before the starting period.
	xhtml = regex.sub(fr"([\p{{Letter}}]){sach.HAIR_SPACE}(\.{sach.WORD_JOINER}{sach.HAIR_SPACE}{sach.WORD_JOINER}…)", r"\1\2", xhtml)

	# Remove spaces between ellipses and endnotes directly after.
	xhtml = regex.sub(fr"…[\s{sach.NO_BREAK_SPACE}]?(<a[^>]+?epub:type=\"noteref\"[^>]*?>)", r"…\1", xhtml, flags=regex.IGNORECASE)

	# Remove word joiners and `nbsp` from `<img>` `alt` attributes.
	for match in regex.findall(fr"alt=\"[^\"]*?[{sach.NO_BREAK_SPACE}{sach.WORD_JOINER}][^\"]*?\"", xhtml):
		xhtml = xhtml.replace(match, match.replace(sach.NO_BREAK_SPACE, " ").replace(sach.WORD_JOINER, ""))

	# Remove word joiners and `nbsp` from `<title>` elements.
	for match in regex.findall(fr"<title>[^<]*?[{sach.NO_BREAK_SPACE}{sach.WORD_JOINER}][^<]*?</title>", xhtml):
		xhtml = xhtml.replace(match, match.replace(sach.NO_BREAK_SPACE, " ").replace(sach.WORD_JOINER, ""))

	# Remove no-break spaces added before `etc.`.
	xhtml = regex.sub(fr"{sach.NO_BREAK_SPACE}(<abbr[^>]*?>etc\.)", r" \1", xhtml)
...
def hyphenate(xhtml: str | EasyXmlTree, language: str | None, ignore_h_tags: bool = False) -> str:
	...
	if language is None:
		try:
			...
			raise sach.InvalidLanguageException("No [attr]@xml:lang[/] or [attr]@lang[/] attribute on [xhtml]<html>[/] element; couldn’t guess file language.") from ex

	# Cope with known missing languages.
	if language in ["en-AU", "en-CA", "en-IE"] :
		language = "en-GB"

	# Vietnamese publishing practice does not hyphenate words at line ends —
	# words are already space-separated syllables — so skip soft-hyphen
	# insertion entirely rather than inserting (incorrect) soft hyphens.
	if vietnamese.is_vietnamese(language):
		return output_xhtml

	language = language.replace("-", "_")

	if language not in pyphen.LANGUAGES:
		raise sach.MissingDependencyException(f"Hyphenator for language [text]{language}[/] not available.\nInstalled hyphenators: {pyphen.LANGUAGES}.")
	...
	for char in text:
		...
		if process:
			if word != "":
				new_word = hyphenator.inserted(word, hyphen=sach.SHY_HYPHEN)
