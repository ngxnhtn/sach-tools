"""
This module implements sách command-line help formatting.
"""
...
class SachHelpFormatter(argparse.HelpFormatter):
	"""
	Format `argparse` auto-generated help text using the sách command-line help style.
	"""
	...
	def _parse_markup(self, text: str) -> Text:
		"""
		Parse help text containing sách Rich-style formatting tags.
		"""
	...
	def _format_metavar(self, action: argparse.Action) -> Text:
		...
		parameter_style = sach.RICH_THEME.styles["parameter"]
		path_style = sach.RICH_THEME.styles["path"]
	...
	def format_help(self) -> str:
		...
		if not sach.should_output_color():
			return text.plain

		output = io.StringIO()
		console = Console(file=output, force_terminal=True, color_system="truecolor", theme=sach.RICH_THEME, width=self._width)
...
def _format_error(self: argparse.ArgumentParser, message: str) -> NoReturn:
	...
	sach.print_error(SachHelpFormatter(self.prog).format_error(message))
	self.exit(2)
