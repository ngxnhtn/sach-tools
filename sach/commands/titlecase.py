...
def titlecase(plain_output: bool) -> int: # pylint: disable=unused-argument
	...
	parser = argparse.ArgumentParser(description="Convert a string to titlecase.", prog="[command]sach[/] [subcommand]titlecase[/]", formatter_class=SachHelpFormatter)
	...
	for line in lines:
		if args.newline:
			print(sach.formatting.titlecase(line))
		else:
			print(sach.formatting.titlecase(line), end="")
