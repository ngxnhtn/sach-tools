...
def build_manifest(plain_output: bool) -> int:
	...
	parser = argparse.ArgumentParser(description="Generate the [xhtml]<manifest>[/] element for the given Vietnamese ebook source directory and write it to the ebook’s metadata file.", prog="[command]sach[/] [subcommand]build-manifest[/]", formatter_class=SachHelpFormatter)
	...
	for directory in args.directories:
		try:
			...
			if args.stdout:
				print(sach_epub.generate_manifest().to_string())
			else:
				nodes = sach_epub.metadata_dom.xpath("/package/manifest")
				if nodes:
					for node in nodes:
						node.replace_with(sach_epub.generate_manifest())
				else:
					for node in sach_epub.metadata_dom.xpath("/package"):
						node.append(sach_epub.generate_manifest())

				# If we have images in the manifest, add or remove some accessibility metadata while we're here.
				access_mode_visual_nodes = sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessMode' and text() = 'visual']")
				access_mode_sufficient_nodes = sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessModeSufficient']")
				accessibility_feature_alt_text_nodes = sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessibilityFeature' and text() = 'alternativeText']")

				if sach_epub.metadata_dom.xpath("/package/manifest/item[starts-with(@media-type, 'image/')]"):
					...
					for filename in sach.get_target_filenames([directory], ".xhtml"):
						dom = sach_epub.get_dom(filename)
					...
					if is_missing_alt_attributes or has_significant_images:
						if not access_mode_visual_nodes:
							sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessMode' and text() = 'textual']")[0].lxml_element.addnext(etree.XML("<meta property=\"schema:accessMode\">visual</meta>"))
						...
						if not accessibility_feature_alt_text_nodes:
							sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessModeSufficient']")[0].lxml_element.addnext(etree.XML("<meta property=\"schema:accessibilityFeature\">alternativeText</meta>"))
				else:
				...
				mathml_accesibility_nodes = sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessibilityFeature' and text() = 'MathML']")
				mathml_described_accesibility_nodes = sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessibilityFeature' and text() = 'describedMath']")

				if sach_epub.metadata_dom.xpath("/package/manifest/item[contains(@properties, 'mathml')]"):
					start_nodes = sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessibilityFeature' and text() = 'alternativeText']")
					if not start_nodes:
						start_nodes = sach_epub.metadata_dom.xpath("/package/metadata/meta[@property='schema:accessModeSufficient']")
