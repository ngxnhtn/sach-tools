"""
This module implements the `sach find-mismatched-diacritics` command.
"""
...
def find_mismatched_diacritics(plain_output: bool) -> int:
	"""
	Entry point for `sach find-mismatched-diacritics`.
	"""

	parser = argparse.ArgumentParser(description="Find words with mismatched diacritics in a set of XHTML files. For example, [text]cafe[/] in one file and [text]café[/] in another.", prog="[command]sach[/] [subcommand]find-mismatched-diacritics[/]", formatter_class=SachHelpFormatter)
	...
	target_filenames = sach.get_target_filenames(args.targets, ".xhtml")
	...
	for filename in target_filenames:
		try:
			with open(filename, "r", encoding="utf-8") as file:
				...
				# If we're in the colophon, remove publisher-site links because a contributor's name might have diacritics.
				if dom.xpath("/html/body//section[contains(@epub:type, 'colophon')]") and dom.xpath("/html/body//a[starts-with(@href, 'http')]"):
					xhtml = regex.sub(r"<a href=\"https?://[^\" ]+</a>", "", xhtml)
			...
			sach.print_error(str(ex) + f" File: [path][link=file://{filename}]{filename}[/][/].", plain_output=plain_output)
			return_code = ex.code
