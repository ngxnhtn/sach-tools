...
def version() -> int:
	...
	# 	pkg_is_editable = Distribution.from_name("sach").origin.dir_info.editable
	direct_url = Distribution.from_name("sach").read_text("direct_url.json")
	if direct_url is None:
		pkg_is_editable = False
	else:
		pkg_is_editable = json.loads(direct_url).get("dir_info", {}).get("editable", False)

	se_version = f"{sach.VERSION} (Việt hoá)"
	if pkg_is_editable:
		se_version += ", editable"
