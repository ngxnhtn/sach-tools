"""
This module implements the `se build-title` command.
"""

import argparse

import sach
from sach.sach_help_formatter import SachHelpFormatter
import sach.easy_xml
import sach.formatting


def build_title(plain_output: bool) -> int:
	"""
	Entry point for `se build-title`.
	"""

	parser = argparse.ArgumentParser(description="Generate the title of an XHTML file based on its headings and update the file’s [xhtml]<title>[/] element.", prog="[command]sach[/] [subcommand]build-title[/]", formatter_class=SachHelpFormatter)
	parser.add_argument("-n", "--no-newline", dest="newline", action="store_false", help="With [flag]--stdout[/], don’t end output with a newline.")
	parser.add_argument("-s", "--stdout", action="store_true", help="Print to stdout instead of writing to the file.")
	parser.add_argument("targets", metavar="[path]TARGET[/]", nargs="+", help="An XHTML file, or a directory containing XHTML files.")
	args = parser.parse_args()

	targets = sach.get_target_filenames(args.targets, ".xhtml")

	if args.stdout and (len(targets) > 1):
		sach.print_error("Multiple targets or directories are not allowed with the [flag]--stdout[/] option.", plain_output=plain_output)
		return sach.InvalidArgumentsException.code

	if not args.newline and not args.stdout:
		sach.print_error("The [flag]--no-newline[/] option can only be used with the [flag]--stdout[/] option.", plain_output=plain_output)

	return_code = 0

	for filename in targets:
		try:
			with open(filename, "rb+") as file:
				dom = sach.easy_xml.EasyXmlTree(file.read())

				title = sach.formatting.generate_title(dom)

				if args.stdout:
					if args.newline:
						print(title)
					else:
						print(title, end="")

				else:
					if title == "":
						sach.print_error(f"Couldn’t deduce title for file: [path][link=file://{filename}]{filename}[/][/].", False, True, plain_output=plain_output)
					else:
						if dom:
							for node in dom.xpath("/html/head/title"):
								node.set_text(title)

							file.seek(0)
							file.write(dom.to_string().encode("utf-8"))
							file.truncate()

		except FileNotFoundError:
			sach.print_error(f"Couldn’t open file: [path][link=file://{filename}]{filename}[/][/].", plain_output=plain_output)
			return_code = sach.InvalidInputException.code
		except sach.SachException as ex:
			sach.print_error(f"File: [path][link=file://{filename}]{filename}[/][/]. {ex}", plain_output=plain_output)
			return_code = ex.code

	return return_code
