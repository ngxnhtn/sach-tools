...
def split_file(plain_output: bool) -> int:
	...
	parser = argparse.ArgumentParser(description="Split an XHTML file into many files at all instances of [xml]<!--se:split-->[/], and include a header template for each file.", prog="[command]sach[/] [subcommand]split-file[/]", formatter_class=SachHelpFormatter)
	...
	parser.add_argument("-s", "--start-at", metavar="INTEGER", type=sach.is_positive_integer, default="1", help="Start numbering chapters at this number, instead of at 1.")
	...
	try:
		with open(filename, "r", encoding="utf-8") as file:
			xhtml = sach.strip_bom(file.read())
	...
	if args.template_file:
		...
		with importlib.resources.files("sach.data.templates").joinpath("chapter-template.xhtml").open("r", encoding="utf-8") as file:
			template_xhtml = file.read()
	...
	for line in xhtml.splitlines():
		if "<!--se:split-->" in line:
			try:
				...
				sach.print_error(f"File has two or more [xml]<!--se:split-->[/] elements in a row: [path][link=file://{filename}]{filename}[/][/].", plain_output=plain_output)
				return sach.InvalidFileException.code
