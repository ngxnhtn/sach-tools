...
from sach.formatting import make_url_safe
...
def build(plain_output: bool) -> int:
	...
	parser = argparse.ArgumentParser(description="Build compatible [path].epub[/] and advanced [path].epub[/] ebooks from a Standard Ebook source directory. Output is placed in the current directory, or the target directory with [flag]--output-dir[/].", prog="[command]sach[/] [subcommand]build[/]", formatter_class=SachHelpFormatter)
	...
	build_cache_root_directory = sach.get_cache_directory() / "build"
	...
	console = Console(width=int(os.environ["COLUMNS"]) if called_from_parallel and "COLUMNS" in os.environ else None, highlight=False, theme=sach.RICH_THEME, force_terminal=force_terminal) # Syntax highlighting will do weird things when printing paths; `force_terminal` prints colors when called from GNU Parallel.

	if args.check_only and (args.check or args.build_kindle or args.build_kobo or args.proof or args.output_dir):
		sach.print_error("The [flag]--check-only[/] option can’t be combined with any other flags except for [flag]--verbose[/].", plain_output=plain_output)
		return sach.InvalidArgumentsException.code

	try:
		max_cache_size = sach.parse_size_string(sach.get_config_value("/configuration/build/@max-cache-size"))
		if max_cache_size <= 0:
			max_cache_size = None

	except sach.InvalidArgumentsException as ex:
		sach.print_error(f"Invalid value for maximum cache size in [link=file://{sach.get_config_file()}]configuration file[/].", plain_output=plain_output)
		return ex.code
	...
	for directory in args.directories:
		...
		try:
			...
			if sach_epub.identifier is None:
				raise sach.InvalidSachEbookException("Couldn’t determine ebook identifier.")

			if args.no_cache:
				ebook_cache_directory = None
			else:
				identifier = regex.sub(r"^https://standardebooks\.org/ebooks/", "", sach_epub.identifier)
				ebook_cache_directory = build_cache_directory / f"{sha256(sach_epub.identifier.encode('utf-8')).hexdigest()}-{make_url_safe(identifier)}"

				try:
					ebook_cache_directory.mkdir(parents=True, exist_ok=True)

					if not os.access(ebook_cache_directory, os.W_OK):
						raise sach.SachException()
			...
			# Now build the ebook!
			sach_epub.build(args.check, args.check_only, args.build_kobo, args.build_kindle, Path(args.output_dir), args.proof, ebook_cache_directory)
		...
		except sach.BuildFailedException as ex:
			exception = ex
		...
		if messages:
			has_output = True
			return_code = sach.BuildFailedException.code
		...
		if max_cache_size is not None:
			...
			if build_cache_directory.is_dir():
				...
				while len(ebook_cache_directories) > 1 and sach.get_directory_size(build_cache_directory) > max_cache_size:
					try:
