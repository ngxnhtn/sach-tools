...
def make_url_safe(plain_output: bool) -> int: # pylint: disable=unused-argument
	...
	parser = argparse.ArgumentParser(description="Make a string URL-safe.", prog="[command]sach[/] [subcommand]make-url-safe[/]", formatter_class=SachHelpFormatter)
	...
	for line in lines:
		if args.newline:
			print(sach.formatting.make_url_safe(line))
		else:
			print(sach.formatting.make_url_safe(line), end="")
