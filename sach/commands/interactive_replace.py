...
def _init_screen(screen: curses.window | None) -> curses.window:
	...
	if curses.has_colors():
		...
		for pair_number, style_name in enumerate(("xhtml", "attr", "val", "error", "dim"), start=1):
			color = sach.RICH_THEME.styles[style_name].color
	...
	try:
		for filepath in sach.get_target_filenames(args.targets, ".xhtml"):
			try:
				...
				return_code = sach.InvalidFileException.code
				continue
