...
def build_svg_titles(plain_output: bool) -> int:
	...
	parser = argparse.ArgumentParser(description="Update or add SVG [xhtml]<title>[/] elements based on the [attr]@alt[/] attributes from the [xhtml]<img>[/] elements.", prog="[command]sach[/] [subcommand]build-svg-titles[/]", formatter_class=SachHelpFormatter)
		...
		try:
			...
			for filename in sach_epub.spine_file_paths:
				...
				for img in dom.xpath(r"/html/body//img[re:test(@src,'\.svg$')]"):
					...
					try:
						svg_dom = sach_epub.get_dom(svg_path)
						...
						if len(title_elements) == 0:
							...
							title_element = sach.easy_xml.EasyXmlElement("<title/>")
						...
						sach.print_error(f"Couldn’t open file: [path][link=file://{svg_path}]{svg_path}[/][/].", plain_output=plain_output)
