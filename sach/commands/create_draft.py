...
USER_AGENT = "Sách toolset <https://giay-phep.local/tools>"
...
def _get_wikipedia_url(string: str, get_nacoaf_uri: bool) -> tuple[str | None, str | None]:
	...
	try:
		...
		raise sach.RemoteCommandErrorException(f"Couldn’t contact Wikipedia: {ex}") from ex
...
def _generate_contributor_string(contributors: list[Contributor], include_xhtml: bool, use_nbsp: bool=False) -> str:
	...
	if len(contributors) == 1:
		if include_xhtml:
			...
			if use_nbsp:
				output += contributors[0].name.replace(" ", sach.NO_BREAK_SPACE)
			else:
		...
		if include_xhtml:
			...
			if use_nbsp:
				output += contributors[0].name.replace(" ", sach.NO_BREAK_SPACE) + " and " + contributors[1].name.replace(" ", sach.NO_BREAK_SPACE)
			else:
		...
		for i, contributor in enumerate(contributors):
			...
			if include_xhtml:
				...
				if use_nbsp:
					output += contributor.name.replace(" ", sach.NO_BREAK_SPACE)
				else:
...
def _create_draft(args: Namespace, plain_output: bool):
	...
	with tempfile.TemporaryDirectory() as directory_name:
		...
		if args.verbose:
			console.print(sach.prep_output("Building ebook structure ...", plain_output), end="")
		...
		# Copy over templates.
		if args.verbose:
			console.print(sach.prep_output("Copying in standard files ...", plain_output), end="")
		...
		if args.white_label:
			...
			_copy_template_file("sach.css", content_path / "epub" / "css")
		...
		# Basic metadata is done, now work on an `SachEpub` object instead of raw files.
		epub = SachEpub(work_path)
		...
		if repo_path.is_dir():
			raise sach.InvalidInputException(f"Directory already exists: [path][link=file://{repo_path}]{repo_path}[/][/].")
		...
		if args.fp_id:
			...
			if args.verbose:
				console.print(sach.prep_output(f"Downloading ebook metadata from [path][link={transcription_url}]{transcription_url}[/][/] ...", plain_output), end="")
			try:
				...
				raise sach.RemoteCommandErrorException(f"Couldn’t download Faded Page ebook metadata page: {ex}") from ex
			...
			if not fp_ebook_url:
				raise sach.RemoteCommandErrorException("Could download ebook metadata, but couldn’t find URL for the ebook HTML.")
			...
			if args.verbose:
				console.print(sach.prep_output(f"Downloading ebook transcription from [path][link={fp_ebook_url}]{fp_ebook_url}[/][/] ...", plain_output), end="")
			try:
				...
				raise sach.RemoteCommandErrorException(f"Couldn’t download Faded Page ebook HTML: {ex}") from ex
			...
			if not fp_html_dom.xpath("/html/head/meta[re:test(@http-equiv, '^Content-Type$', 'i') and re:test(@content, 'text/html;\\s*charset=utf-8', 'i')]", namespaces=XPATH_NAMESPACES):
				raise sach.RemoteCommandErrorException("Tried to download Faded Page ebook HTML, but response doesn't look like a Faded Page ebook.")
			...
			try:
				...
				transcription_ebook_html = sach.strip_bom(fixed_external_ebook_html)
			except Exception as ex:
				raise sach.InvalidEncodingException(f"Couldn’t determine text encoding of Faded Page HTML file: {ex}") from ex
		...
		if args.pg_id:
			...
			try:
				...
				raise sach.RemoteCommandErrorException(f"Couldn’t download Project Gutenberg ebook metadata page: {ex}") from ex
			...
			if args.verbose:
				console.print(sach.prep_output(f"Downloading ebook transcription from [path][link={pg_ebook_url}]{pg_ebook_url}[/][/] ...", plain_output), end="")
			try:
				response = requests.get(pg_ebook_url, timeout=60, headers={'User-Agent': USER_AGENT})
				transcription_ebook_html = response.text

			except Exception as ex:
				raise sach.RemoteCommandErrorException(f"Couldn’t download Project Gutenberg ebook HTML: {ex}") from ex

			try:
				fixed_external_ebook_html = fix_text(transcription_ebook_html, uncurl_quotes=False)
				transcription_ebook_html = sach.strip_bom(fixed_external_ebook_html)
			except Exception as ex:
				raise sach.InvalidEncodingException(f"Couldn’t determine text encoding of Project Gutenberg HTML file: {ex}") from ex
		...
		if transcription_subjects:
			...
			for subject in transcription_subjects:
				...
				try:
					...
					subject_node = sach.easy_xml.EasyXmlElement(element)
					subject_node.set_attr("id", f"subject-{i}")
					subject_node.set_text(subject)
					authority_node = sach.easy_xml.EasyXmlElement(f"<meta property=\"authority\" refines=\"#subject-{i}\">LCSH</meta>")
					term_node = sach.easy_xml.EasyXmlElement(f"<meta property=\"term\" refines=\"#subject-{i}\">{loc_id}</meta>")
					...
					raise sach.RemoteCommandErrorException(f"Couldn’t connect to [url][link=https://id.loc.gov]https://id.loc.gov[/][/]: {ex}") from ex
		...
		if transcription_ebook_html:
			if args.verbose:
				console.print(sach.prep_output("Cleaning transcription ...", plain_output), end="")
			...
			if args.fp_id:
				try:
					...
					for node in dom.xpath("/html/head | /html//script"):
						easy_node = sach.easy_xml.EasyXmlElement(node)
						easy_node.remove()
			...
			if transcription_producers:
				...
				for producer in transcription_producers:
					...
					contributor_node = sach.easy_xml.EasyXmlElement(element)
					contributor_node.set_attr("id", transcriber_id)
					contributor_file_as_node = sach.easy_xml.EasyXmlElement(f"<meta property=\"file-as\" refines=\"#{transcriber_id}\">TRANSCRIBER_{i}_SORT</meta>")
					contributor_homepage_node = None
					contributor_role_node = sach.easy_xml.EasyXmlElement(f"<meta property=\"role\" refines=\"#{transcriber_id}\" scheme=\"marc:relators\">trc</meta>")
					...
					if "Distributed Proofreaders Canada" in producer:
						...
						contributor_homepage_node = sach.easy_xml.EasyXmlElement(f"<link href=\"https://www.pgdpcanada.net/\" refines=\"#{transcriber_id}\" rel=\"schema:url\"/>")
						...
						contributor_homepage_node = sach.easy_xml.EasyXmlElement(f"<link href=\"https://www.pgdp.net/\" refines=\"#{transcriber_id}\" rel=\"schema:url\"/>")
					...
					if "David Widger" in producer:
						contributor_lccn_node = sach.easy_xml.EasyXmlElement(f"<link href=\"https://id.loc.gov/authorities/names/no2011017869.html\" refines=\"#{transcriber_id}\" rel=\"schema:sameAs\"/>")
			...
			try:
				...
				raise sach.InvalidFileException(f"Couldn’t write to ebook directory: {ex}") from ex
		...
		with open(content_path / "epub" / "text" / "titlepage.xhtml", "r+", encoding="utf-8") as file:
			...
			file.seek(0)
			file.write(sach.formatting.format_xhtml(titlepage_xhtml))
			file.truncate()
		...
		if not args.white_label:
			...
			with open(content_path / "epub" / "text" / "colophon.xhtml", "r+", encoding="utf-8") as file:
				...
				colophon_xhtml = colophon_xhtml.replace("SACH_SLUG", epub.generate_url_slug())
		...
		metadata_xml = sach.formatting.format_opf(metadata_xml)
		...
		placeholder_values = {name.replace("-", "_").upper(): value for name, value in sach.get_config_attributes("/configuration/create-draft").items() if name not in ignored_attributes}
		...
		if args.verbose:
			console.print(sach.prep_output("Initializing git repository ...", plain_output), end="")
	...
	if args.pg_id and transcription_ebook_html and not is_external_html_parsed:
		raise sach.InvalidXhtmlException(f"Couldn’t parse Project Gutenberg ebook source; this is usually due to invalid HTML in the ebook. The raw text was saved to [path][link={transcription_local_path}]{transcription_local_path}[/][/].")
...
def create_draft(plain_output: bool) -> int:
	...
	parser = argparse.ArgumentParser(description="Create a skeleton of a new Standard Ebook in the current directory.", prog="[command]sach[/] [subcommand]create-draft[/]", formatter_class=SachHelpFormatter)
	...
	source_group.add_argument("-f", "--fp-id", dest="fp_id", type=sach.is_positive_integer, help="The Faded Page ID number of the ebook to download.")
	...
	if args.email is None:
		default_email = sach.get_config_value("/configuration/create-draft/@default-email")
		if default_email:
	...
	try:
		...
		if ":" in args.title:
			console.print(sach.prep_output("Titles should not include a subtitle, as subtitles are separate metadata elements in [path]content.opf[/]. Are you sure you want to continue? \\[y/N]", plain_output))
