...
Strictly speaking, the `SachEpub.generate_toc()` function should be a class member of `SachEpub`. But the function is very big and it makes editing easier to put in a separate file.
"""
...
class TocItem:
	...
	def toc_link(self) -> str:
		...
		out_string = ""
		if not self.title:
			raise sach.InvalidInputException(f"Couldn’t find title in: [path][link=file://{self.epub_content_path}/text/{self.file_link}]{self.file_link}[/][/].")
...
def add_landmark(dom: EasyXmlTree, textf: str, landmarks: list[TocItem]) -> None:
	...
	if not sections:
		raise sach.InvalidInputException("Couldn’t locate first [xhtml]<section>[/], [xhtml]<article>[/], or [xhtml]<nav>[/].")
	epub_type = sections[0].get_attr("epub:type")
	bodys = dom.xpath("//body")
	if not bodys:
		raise sach.InvalidInputException("Couldn’t locate [xhtml]<body>[/].")
...
def output_toc(item_list: list[TocItem], landmark_list: list[TocItem], work_title: str, language: str) -> str:
	...
	if len(item_list) < 2:
		raise sach.InvalidInputException("Too few ToC items found.")

	with importlib.resources.files("sach.data.templates").joinpath("toc.xhtml").open("rb") as file:
		toc_dom = sach.easy_xml.EasyXmlTree(file.read())

	# There should be exactly two nav sections.
	navs = toc_dom.xpath("//nav")

	if len(navs) < 2:
		raise sach.InvalidInputException("Existing ToC has too few nav sections.")
	...
	return sach.formatting.format_xhtml(toc_dom.to_string())
...
def process_headings(dom: EasyXmlTree, textf: str, toc_list: list[TocItem], single_file: bool, single_file_without_headers: bool, content_path: Path) -> None:
	...
	if not heads:  # May be a dedication or an epigraph, with no heading element.
		...
		if content_item: # Check to see if it has a `data-parent` attribute, if so, we'll use that to determine depth.
			...
			raise sach.InvalidInputException(f"Unable to find heading or content item (p, header or img) in file: [path][link=file://{textf}]{textf}[/][/].")
		try:
...
def process_a_heading(node: EasyXmlElement, textf: str, is_toplevel: bool, single_file: bool, content_path: Path) -> TocItem:
	...
	if not epub_type and node.tag in ["h1", "h2", "h3", "h4", "h5", "h6"]:
		...
		if parent:
			...
			raise sach.InvalidInputException(f"Heading without parent in file: [path][link=file://{textf}]{textf}[/][/].")
		return toc_item
	if epub_type:
		...
		if "z3998:roman" in epub_type:
			...
			try:
				...
				raise sach.InvalidInputException(f"Heading tagged as roman numeral is invalid: {error_string} in [path][link=file://{textf}]{textf}[/][/].") from err
...
def get_book_division(node: EasyXmlElement) -> BookDivision:
	...
	if not parent_sections:  # Couldn't find a parent, so throw an error.
		raise sach.InvalidInputException("Couldn’t find division type.")
...
def process_all_content(self: 'SachEpub', file_list: list[Path]) -> tuple[list[TocItem], list[TocItem]]:
	"""
	...
	for textf in file_list:
		try:
			...
			raise sach.InvalidFileException(f"Couldn’t open file: [path][link=file://{textf}]{textf}[/][/]: {ex}") from ex
...
def generate_toc(self: 'SachEpub') -> str:
	"""
	Entry point for `SachEpub.generate_toc()`.
	"""

	if not self.language:
		raise sach.InvalidSachEbookException("Couldn’t determine ebook language.")
