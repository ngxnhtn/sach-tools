...
import sach
import sach.commands
from sach.sach_help_formatter import SachHelpFormatter


def get_commands() -> list[str]:
	"""
	Helper function to generate a list of available commands from all of the submodules in the sach.cmd package
	"""

	commands: list[str] = []
	for module_info in pkgutil.iter_modules(sach.commands.__path__):
...
def main() -> None:
	...
	This function delegates subcommands (like `se typogrify`) to individual submodules under `sach.commands`.
	"""

	try:
		# If we're asked for the version, short circuit and exit.
		if len(sys.argv) == 2 and (sys.argv[1] == "-v" or sys.argv[1] == "--version"):
			module = importlib.import_module("sach.commands.version")
		...
		parser = argparse.ArgumentParser(description="The entry point for the Vietnamese ebook toolset.", formatter_class=SachHelpFormatter)
		...
		sach.COLOR_OUTPUT = args.color_output
		...
		command_module = f"sach.commands.{command_name}"
