...
import sach.vietnamese
from sach.easy_xml import EasyXmlTree, EasyXmlElement
...
def semanticate(xhtml: str) -> str:
	...
	xhtml = regex.sub(fr"(?<!\.|\B)(St\.)(?={sach.NO_BREAK_SPACE})", r"""<abbr epub:type="z3998:name-title">St.</abbr>""", xhtml)
	...
	xhtml = regex.sub(fr"""<span epub:type="z3998:roman">([IVX])</span>{sach.WORD_JOINER}⸺""", fr"""\1{sach.WORD_JOINER}⸺""", xhtml)

	# Add abbreviations around some SI measurements.
	xhtml = regex.sub(r"([0-9]+)\s*([cmk][mgl])\b", fr"\1{sach.NO_BREAK_SPACE}<abbr>\2</abbr>", xhtml)

	# Add abbreviations around Imperial measurements.
	xhtml = regex.sub(r"(?<![\$£0-9,])([0-9½¼⅙⅚⅛⅜⅝⅞]+)\s*(ft|yd|yds|mi|pt|qt|gal|oz|lb|lbs)\.?(\W)", fr"\1{sach.NO_BREAK_SPACE}<abbr>\2.</abbr>\3", xhtml)

	# Handle `in.` separately to require a period, because with an optional period there are too many false positives.
	xhtml = regex.sub(r"(?<![\$£0-9,])([0-9½¼⅙⅚⅛⅜⅝⅞]+)\s*in\.(\b|[\s,:;!?])", fr"\1{sach.NO_BREAK_SPACE}<abbr>in.</abbr>\2", xhtml)

	# Fix some possible errors introduced by the above.
	xhtml = regex.sub(fr"((?:[Nn]o\.|[Nn]umber)\s[0-9]+){sach.NO_BREAK_SPACE}<abbr>in\.</abbr>", r"\1 in", xhtml)

	# Tweak some other Imperial measurements.
	xhtml = regex.sub(r"([0-9]+)\s*m\.?p\.?h\.?", fr"\1{sach.NO_BREAK_SPACE}<abbr>mph</abbr>", xhtml, flags=regex.IGNORECASE)
	xhtml = regex.sub(r"([0-9]+)\s*h\.?p\.?", fr"\1{sach.NO_BREAK_SPACE}<abbr>hp</abbr>", xhtml, flags=regex.IGNORECASE)
	...
	for node in dom.xpath("/html/head/title"):
		replacement_node = sach.easy_xml.EasyXmlElement(f"<title>{escape_xml(node.inner_text())}</title>")
...
def get_word_count(xhtml: str) -> int:
	...
	xhtml = regex.sub(fr"[\p{{Letter}}0-9][\-\'’‘\,\.\/{sach.NO_BREAK_HYPHEN}{sach.SHY_HYPHEN}][\p{{Letter}}0-9]", "x", xhtml)

	# Replace some formatting characters.
	xhtml = regex.sub(fr"[…–—―{sach.NO_BREAK_SPACE}‘’“”\\{{\\}}\\(\\)]", " ", xhtml)

	# Precompose (NFC) Vietnamese so that decomposed tone marks don't split a
	# single word into fragments, then count word units.
	xhtml = sach.vietnamese.normalize(xhtml)
...
def _format_style_elements(tree: etree.Element):
	...
	try:
		for node in tree.xpath("//svg:style", namespaces={"xhtml": "http://www.w3.org/1999/xhtml", "svg": "http://www.w3.org/2000/svg"}):
			...
			node.text = "\n" + css + "\n" + indent
	except sach.InvalidCssException as ex:
		raise ex
	except Exception as ex:
		raise sach.InvalidCssException(f"Couldn’t parse CSS: {ex}")
...
def format_xml(xml: str) -> str:
	...
	try:
		...
		raise sach.InvalidXmlException(f"Couldn’t parse XML file: {ex}")
...
def format_se_lint_ignore(xml: str) -> str:
	...
	try:
		dom = sach.easy_xml.EasyXmlTree(xml)
...
def format_xhtml(xhtml: str) -> str:
	...
	try:
		...
		raise sach.InvalidXhtmlException(f"Couldn’t parse XHTML file: {ex}")
...
def format_opf(xml: str) -> str:
	...
	try:
		...
		raise sach.InvalidXmlException(f"Couldn’t parse OPF file: {ex}")
...
def format_svg(svg: str) -> str:
	...
	try:
		...
		raise sach.InvalidXmlException(f"Couldn’t parse SVG file: {ex}")
...
def _format_css_rules(content: list[Node], indent_level: int) -> str:
	...
	for token in tinycss2.parser.parse_rule_list(content):
		if isinstance(token, ParseError):
			raise sach.InvalidCssException(f"Couldn’t parse CSS: {token.message}")
...
def titlecase(text: str, language: str | None = None) -> str:
	...
	text = regex.sub(fr"[^\S{sach.HAIR_SPACE}{sach.NO_BREAK_SPACE}]+", " ", text)
	...
	text = regex.sub(fr"([—–]{sach.WORD_JOINER}?)([\p{{Lowercase_Letter}}])", lambda result: result.group(1) + result.group(2).upper(), text)
...
def simplify_css(css: str) -> str:
	...
	for line in lines:
		...
		for pseudo_class_to_simplify in sach.PSEUDO_CLASSES_TO_SIMPLIFY:
...
def generate_title(xhtml: str | EasyXmlTree) -> str:
	...
	if hgroup_elements:
		...
		if closest_parent_sections:
			...
			raise sach.InvalidSachEbookException("No [xhtml]<section>[/] or [xhtml]<article>[/] element for [xhtml]<hgroup>[/].")
		...
		try:
			...
			raise sach.InvalidSachEbookException("Couldn’t find title in [xhml]<hgroup>[/].") from ex
	...
	title = regex.sub(fr"[{sach.NO_BREAK_SPACE}]", " ", title)
	title = regex.sub(fr"[{sach.WORD_JOINER}{sach.ZERO_WIDTH_SPACE}]", "", title)
	...
	title = regex.sub(fr"[^\S{sach.HAIR_SPACE}]+", " ", title)
...
def generate_colophon_timestamp(timestamp: datetime) -> str:
	...
	formatted_timestamp = f"{timestamp:%B %e, %Y, %I:%M{sach.NO_BREAK_SPACE}<abbr class=\"eoc\">%p</abbr>}".replace(" 0", " ")
	formatted_timestamp = regex.sub(r"\s+", " ", formatted_timestamp).replace("AM", "a.m.").replace("PM", "p.m.").replace(" <abbr", f"{sach.NO_BREAK_SPACE}<abbr")
...
def find_unexpected_ids(dom: EasyXmlTree, no_endnotes: bool = False) -> list[tuple[EasyXmlElement, str]]:
	...
	# 3. Exclude `<p>` children of `<hgroup>` as they are really titles and not `<p>` per sach.
	line_number = 0
	endnote_number = 0
